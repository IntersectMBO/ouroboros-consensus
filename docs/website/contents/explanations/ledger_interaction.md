...
has. Evaluating a ledger rule is essentially running an `STS` transition
that produces a new state or throws failures.
...
(Babbage, Conway, ...) is in `ouroboros-consensus-protocol`, hence why it is not
...
transitions (which implies rewards), or pulsing calculations that are spread
over time.
...
are some checks that cannot change their outcome, such as cryptographic hashes
checking. For that purpose, Ledger exposes the function
...
which has the same type as `applyTx`.
...
is would be absurd to ask whether `B` is valid or not.
...
which returns a new state (`NewEpochState era`), a (empty if the block is valid)
list of failures and a list of generated events.
...
always. This introduces a duality among `applyBlock` and `reapplyBlock`, in
which `reapplyBlock` skips as many checks as possible and cannot fail.

We will use `reapplyBlock` to apply the blocks from Genesis (or the snapshot we
...
layer that uses impure code, is the one responsible of storing and managing such
set so that it can be stored in persistent storage, lowering the memory
...
class, which is capable to translating types from the previous era to the
current one, using a
