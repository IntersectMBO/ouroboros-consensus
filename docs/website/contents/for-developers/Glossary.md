...
In some [slots](#slot), the protocol allows a block-producing [node](#node) to contribute to the [chain](#chain); it does so by minting a new block.
A block at least identifies its [issuer](#issuer), its own real [point](#point), and the hash of the preceding block.
...
A [node](#node) is a party that can participate in the [network](#net).
...
## Block validity and ledger rules
...
The accumulated result (of a [prefix](#prefix/tip/etc-and-fragment)) of a [chain](#chain) (which necessarily started from the [genesis block](#genesis-block)).
...
A [block body](#header-and-body) is just a sequence of transactions.
Each one modifies the [ledger state](#ledger-state) in a way determined by the [ledger rules](#block-validity-and-ledger-rules).
