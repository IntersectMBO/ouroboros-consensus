...
import Control.Monad.Except qualified as Exn
import Data.Functor ((<&>))
import Data.Functor.Identity (runIdentity)
import Data.IORef (modifyIORef', newIORef, readIORef, writeIORef)
import Data.Proxy (Proxy (Proxy))
import System.Random qualified as R
import System.Timeout qualified as IO (timeout)
import Test.Ouroboros.Consensus.ChainGenerator.Adversarial
  ( genPrefixBlockCount
  )
import Test.Ouroboros.Consensus.ChainGenerator.Adversarial qualified as A
import Test.Ouroboros.Consensus.ChainGenerator.BitVector qualified as BV
import Test.Ouroboros.Consensus.ChainGenerator.Counting qualified as C
import Test.Ouroboros.Consensus.ChainGenerator.Honest qualified as H
...
import Test.Ouroboros.Consensus.ChainGenerator.RaceIterator qualified as RI
import Test.Ouroboros.Consensus.ChainGenerator.Slot (E (SlotE))
import Test.Ouroboros.Consensus.ChainGenerator.Slot qualified as S
import Test.Ouroboros.Consensus.ChainGenerator.Some qualified as Some
...
tests =
  ...
  , TT.localOption (TT.QuickCheckMaxSize 6) $
      TT.testProperty
  ...
  arbitrary = unsafeMapSuchThatJust $ do
    ...
    case Exn.runExcept $ A.checkAdversarialRecipe testRecipeA of
      ...
      Right testRecipeA' -> do
        pure $
          Just $
            SomeTestAdversarial Proxy Proxy $
              TestAdversarial
                { testAscA
                , testAscH
                , testRecipeA
...
calculateStability (Scg s) schedA =
  C.withWindow
    ...
    (C.Lbl @AdvStabLbl)
    (C.Count 0)
    (C.Count $ firstActive + theBlockItself + s)
 where
  H.ChainSchema winA vA = schedA
   ...
   where
    ...
    deltaIncrementFromRecipe (A.SomeCheckedAdversarialRecipe _ r) =
      let H.ChainSchema _ v = A.carHonest r
       ...
       in case BV.findIthEmptyInV S.inverted sv (C.Count (k + 1)) of
    ...
    IO.timeout
      ...
      >>= \case
        ...
        Nothing ->
          ...
          ((,) <$> readIORef catch <*> readIORef counter) <&> \((seedA, schedA'), n) ->
            id
