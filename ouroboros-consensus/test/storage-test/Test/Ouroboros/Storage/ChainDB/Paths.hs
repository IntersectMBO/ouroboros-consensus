...
genSuccessor ::
  ...
  Gen TestBody ->
  Gen TestBlock
genSuccessor (prevHeaderFields, prevIsEBB, prevChainLength) genBody =
  frequency
    [
...
generateDisconnectedHeaders (chain, mbAnchorHdr) =
 ...
 where
  ...
  dropConnectingBlock (ChainDiff _ suffix, hdrs) = do
    connectingPt <-
      either (const Nothing) (Just . castPoint . blockPoint) $
        AF.last suffix
