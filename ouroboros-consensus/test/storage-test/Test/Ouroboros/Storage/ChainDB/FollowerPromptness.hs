...
import Ouroboros.Consensus.Storage.ChainDB.API.Types.InvalidBlockPunishment qualified as Punishment
import Ouroboros.Consensus.Storage.ChainDB.Impl qualified as ChainDBImpl
  ...
  counterexample ("Trace:\n" <> unlines (ppTrace <$> traceByTime)) $
    counterexample (condense fpts) $
      counterexample ("Instruction timings: " <> condense followerInstrTimings) $
        counterexample
          ("Failed to pipeline: " <> condense notPipelined)
          (null notPipelined)
          .&&. counterexample
            ("Not processed: " <> condense unprocessed)
            (null unprocessed)
 where
...
runFollowerPromptnessTest FollowerPromptnessTestSetup{..} = withRegistry \registry -> do
  ...
  _ <-
    forkLinkedThread registry "Follower listener" $
      forever $
