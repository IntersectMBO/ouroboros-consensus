...
data Model blk = Model
  ...
  , currentLedger :: ExtLedgerState blk NoTables
  , initLedger :: ExtLedgerState blk NoTables
...
deriving instance
  ...
  , ToExpr (ExtLedgerState blk NoTables)
  , StandardHash blk
  , Show blk
  ) =>
  ToExpr (Model blk)
...
deriving instance
  (LedgerSupportsProtocol blk, Show (LedgerState blk NoTables), Show blk) => Show (Model blk)
...
addBlock ::
  forall blk.
  (LedgerSupportsProtocol blk, TrivialTables ExtLedgerState blk) =>
  TopLevelConfig blk ->
  blk ->
  Model blk ->
  Model blk
...
chainSelection ::
  forall blk.
  ( TrivialTables ExtLedgerState blk
  , LedgerSupportsProtocol blk
  ) =>
  TopLevelConfig blk ->
  Model blk ->
...
chainSelection cfg m =
 ...
 where
  ...
  invalid' :: InvalidBlocks blk
  candidates :: [(Chain blk, ExtLedgerState blk NoTables)]
  ...
  newChain :: Chain blk
  newLedger :: ExtLedgerState blk NoTables
  (newChain, newLedger) =
...
addBlockPromise ::
  forall m blk.
  (LedgerSupportsProtocol blk, MonadSTM m, TrivialTables ExtLedgerState blk) =>
  TopLevelConfig blk ->
  blk ->
  Model blk ->
...
validate cfg Model{initLedger, invalid} chain =
 ...
 where
  ...
  go ledger validPrefix = \case
    ...
    b : bs' -> case runExcept (tickThenApply OmitLedgerEvents (ExtLedgerCfg cfg) b (convertTrivialTables ledger)) of
      ...
      Right ledger' -> go (convertTrivialTables ledger') (validPrefix :> b) bs'
...
validChains ::
  ...
  (InvalidBlocks blk, [(Chain blk, ExtLedgerState blk NoTables)])
