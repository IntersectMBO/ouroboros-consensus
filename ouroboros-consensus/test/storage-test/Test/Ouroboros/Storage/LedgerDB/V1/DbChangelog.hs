...
prop_pushExpectedLedger setup@ChainSetup{..} =
    classify (chainSetupSaturated setup) "saturated" $
      conjoin [
          l === convertMapKind (refoldLedger OmitLedgerEvents cfg (expectedChain o) (convertMapKind TestBlock.testInitLedger))
...
data Operation l = Extend (l DiffMK) | Prune LedgerDBPruneTip
...
prop_flushingSplitsTheChangelog setup = isNothing toFlush .||.
    ...
    .&&. DS.fromAntiDiff (DS.cumulativeDiff diffs) === toFlushDiffs <> DS.fromAntiDiff (DS.cumulativeDiff toKeepDiffs)
    )
  where
...
prop_pruningLeavesAtMostMaxRollbacksVolatileStates setup sp@(SecurityParam k) =
  ...
  where
    dblog = resultingDbChangelog setup
    dblog' = DbChangelog.prune (LedgerDBPruneTip sp) dblog
...
genOperations slotNo nOps = gosOps <$> execStateT (replicateM_ nOps genOperation) initState
  where
    ...
    genPrune = Prune <$> lift (oneof [
        pure LedgerDBPruneTipZero
      , LedgerDBPruneTip . SecurityParam . unsafeNonZero <$> chooseEnum (1, 10)
      ])
