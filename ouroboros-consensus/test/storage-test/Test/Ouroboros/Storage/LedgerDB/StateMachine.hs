    ...
    ioProperty $
      bracket setup cleanup $
        \env0 -> do
          ...
          (actual, expected) <- checkNoLeakedHandles env1
          let p2 =
                counterexample
...
-- runStateTPropertyM ::
--   Environment ->
--   QC.PropertyM (StateT Environment IO) a ->
--   Property
-- runStateTPropertyM env propM =
--   QC.monadic (QC.ioProperty . Monad.void . flip runStateT env) propM
...
--       lift $ closeLedgerDB testInternals >> clean
...
checkNoLeakedHandles :: Environment -> IO (NumOpenHandles, NumOpenHandles)
