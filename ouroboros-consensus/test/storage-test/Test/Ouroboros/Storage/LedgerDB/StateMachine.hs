...
dbStreamAPI secParam chainDb =
  ...
  where
    ...
    streamAfter tip k = do
        pts <- atomically $ reverse . drop (fromIntegral $ BT.unNonZero $ maxRollbacks secParam) <$> readTVar dbChain
        case tip' of
          NotOrigin pt
            | pt `L.notElem` pts
            -> k $ Left pt
...
instance RunModel Model (StateT Environment IO) where
  ...
  postcondition (Model chain secParam, _) GetState _ (imm, vol) =
    let volSt = either forgetLedgerTables (forgetLedgerTables . snd) (AS.head chain)
        immSt = either forgetLedgerTables (forgetLedgerTables . snd) (AS.head (AS.dropNewest (fromIntegral $ BT.unNonZero $ maxRollbacks secParam) chain))
    in do
