    ...
    insert tok val UTxTok{utxtok, utxhist} =
      if tok `Set.member` utxhist
        then Left $ TokenWasAlreadyCreated tok
          ...
          Right $
            UTxTok
              { utxtok = Map.insert tok val utxtok
...
mockUpdateLedger ::
  ...
  Mock ->
  (Either (ExtValidationError TestBlock) a, Mock)
...
runMock cmd initMock =
 ...
 where
  ...
  go (Drop n) mock =
    -- the model does not track checksums, but ask for it for
    -- consistency with the real thing
    go (Restore DoDiskSnapshotChecksum) $
      mock
...
runDB standalone@DB{..} cmd =
 ...
 where
  ...
  go hasFS (Drop n) = do
    ...
    -- always attempt to check the snapshot checksum. The current implementation
    -- will successfully restore even if the checksum is missing
    -- (producing a warning trace).
    go hasFS (Restore DoDiskSnapshotChecksum)
  ...
  upd f g = do
    ...
    case mDB' of
      ...
      Right db' -> do
        atomically $ writeTVar dbState (f rs, db')
...
lockstep ::
  ...
  Resp :@ r ->
  Event r
lockstep m@(Model _ hs) cmd (At resp) =
  Event
    { eventBefore = m
    , eventCmd = cmd
    , eventResp = At resp
...
generator secParam (Model mock hs) =
  Just $
    QC.oneof $
      concat
        [ withoutRef
 ...
 where
  ...
  possibleCorruptions = concatMap aux hs
   where
    aux ::
      (Reference DiskSnapshot Symbolic, MockSnap) ->
 ...
 where
  ...
  tagRestore mST =
    fmap (TagRestore mST . rangeK k) $ C.maximum $ \ev ->
      let mock = modelMock (eventBefore ev)
       in case eventCmd ev of
