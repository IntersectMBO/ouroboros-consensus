    ...
    case t of
      ...
      Just timeSinceLast ->
        counterexample "have previously taken a snapshot" $
          isDisjunctionOf
            (shouldTakeSnapshot ts `named` "the decision")
 ...
 where
  ...
  systemChecksHowMuchTimeHasPassed timeSinceLast =
    case snapshotInterval of
      DefaultSnapshotInterval ->
        (timeSinceLast >= kTimes2)
          `named` "time since last is greater then 2 * k seconds if snapshot interval is set to default"
      RequestedSnapshotInterval interval ->
        (timeSinceLast >= interval)
          `named` "time since last is greater then explicitly requested interval"
  ...
  systemChecksHowManyBlocksWereProcessed timeSinceLast =
   ...
   where
    msg =
      unwords
        [ "we have processed"
        , show minBlocksBeforeSnapshot
        , "blocks and it's been more than"
        , show minSecondsBeforeSnapshot
