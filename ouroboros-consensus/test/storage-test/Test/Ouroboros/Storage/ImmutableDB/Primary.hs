...
import System.FS.Sim.STM qualified as Sim
...
prop_filledSlots_isFilledSlot (TestPrimaryIndex chunkInfo chunk idx _slot) =
  conjoin
...
runFS m =
  try (Sim.runSimFS Mock.empty m) >>= \case
...
instance Arbitrary TestPrimaryIndex where
  ...
  shrink test =
    concat
      [ flip map (shrinkIndex $ testPrimaryIndex test) $ \i ->
          test
            { testPrimaryIndex = i
            }
      , flip map (shrink $ testChunkNo test) $ \chunkNo' ->
          test
            { testChunkNo = chunkNo'
            , testPrimaryIndex =
