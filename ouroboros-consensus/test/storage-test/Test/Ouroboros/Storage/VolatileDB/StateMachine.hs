...
shrinkerImpl m (At (CmdErr cmd mbErr)) =
  fmap At $
...
runDB env@VolatileDBEnv{varDB, args = VolatileDbArgs{volHasFS = SomeHasFS hasFS}} cmd =
  readTVarIO varDB >>= \db -> case cmd of
    ...
    Corruption corrs ->
      withClosedDB $
        forM_ corrs $
          \(corr, file) -> corruptFile hasFS corr file
...
tag ls =
 ...
 where
  ...
  tagGetBlockComponentNothing = successful $ \ev r -> case r of
    MbAllComponents Nothing
      | GetBlockComponent{} <- getCmd ev ->
          Left TagGetNothing
  ...
  tagGarbageCollect keep bids mbGCed = successful $ \ev suc ->
    if not keep
      then Right $ tagGarbageCollect keep bids mbGCed
