...
withFreshSQLiteDb action = do
  ...
  bracket (createTempDirectory sysTmp "leios-test") removeDirectoryRecursive $ \tmpDir ->
    withLeiosDBSQLite nullTracer (tmpDir <> "/test.db") action
...
withFreshSQLiteFile action = do
  ...
  bracket
    ...
    ( \tmpDir -> do
        ...
        withLeiosDBSQLite nullTracer dbPath (action dbPath)
    )
