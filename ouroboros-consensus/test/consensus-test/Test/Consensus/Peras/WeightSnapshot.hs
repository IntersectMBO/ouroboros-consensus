    ...
    $ conjoin
      [ conjoin
          [ counterexample ("Incorrect weight for " <> condense pt) $
              weightForPoint pt === weightBoostOfPoint snap pt
          | pt <- tsPoints
          ]
      , conjoin
          [ counterexample ("Incorrect weight for " <> condense frag) $
              weightForFrag frag === weightBoostOfFragment snap frag
          | frag <- tsFragments
          ]
      ]
 where
  ...
  weightForPoint :: Point TestBlock -> PerasWeight
  weightForPoint pt = Map.findWithDefault mempty pt tsWeights

  weightForFrag :: AnchoredFragment TestBlock -> PerasWeight
  weightForFrag frag =
    foldMap
      (weightForPoint . blockPoint)
