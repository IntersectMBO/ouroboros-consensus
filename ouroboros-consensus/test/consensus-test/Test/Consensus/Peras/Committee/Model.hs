...
class
  (Integral (NumSeats st), Ord (NumSeats st)) =>
  IsNumSeats (st :: StakeType)
  where
  type NumSeats st
...
class
  (Num (Stake so st), Ord (Stake so st)) =>
  IsStake (so :: StakeRole) (st :: StakeType)
  where
  data Stake so st

  stakeToRational :: Stake so st -> Rational
  rationalToStake :: Rational -> Stake so st
...
--
-- NOTE: type parameters are arranged so that on
coerceStake ::
  forall so' st' so st.
  ( IsStake so st
  , IsStake so' st'
...
type StakeDistr (so :: StakeRole) (st :: StakeType) = Map VoterId (Stake so st)
...
weightedFaitAccompliPersistentSeats globalNumSeats stakeDistr
 ...
 where
  ...
  (persistentVotersWithStake, residualVotersWithStake) =
    bimap
      (mapStakes (coerceStake @Voting @Persistent))
      (mapStakes (coerceStake @Ledger @Residual))
      $ span
...
localSortition ::
  (IsStake 'Ledger st, IsStake 'Voting st, IsNumSeats st) =>
  RandomGen rng =>
...
localSortition rng0 numSeats stakeDistr =
  -- TODO: what happens if stake distribution is of size 0, or numSeats is 0?
...
makeStakeDistrVotingGlobal persistent residual =
  Map.unionWith
    (+)
    (coerceStake @Voting @Global <$> persistent)
    (coerceStake @Voting @Global <$> residual)

-- ** Random sampling helpers

-- | Poisson sampling using Knuth's algorithm
--
-- NOTE: please see commented code below for a version that uses 'mwc-random'.
poisson :: RandomGen rng => Double -> rng -> (Natural, rng)
poisson lambda rng0 =
  first fromIntegral $
    go 0 1 rng0
 where
  go :: RandomGen rng => Int -> Double -> rng -> (Int, rng)
  go k p rng
    | p <= exp (-lambda) =
        (k - 1, rng)
    | otherwise =
...
-- explicit seed.
--
-- TODO: replace the definition above with this one after updating 'mwc-random'
-- to a version that includes the Poisson distribution (>=15.3.0).
--
-- poisson :: RandomGen rng => Double -> rng -> (Natural, rng)
-- poisson lambda rng0
--   | lambda < 0 =
...
-- TODO: can we write a DerivingVia helper here?

instance IsNumSeats 'Global where
  type NumSeats 'Global = Natural

instance IsNumSeats 'Persistent where
  type NumSeats 'Persistent = Natural

instance IsNumSeats 'Residual where
  type NumSeats 'Residual = Natural

instance IsStake 'Ledger 'Global where
  newtype Stake 'Ledger 'Global = StakeLedgerGlobal Rational
    deriving stock (Eq, Ord, Show, Generic)
    deriving newtype Num
  stakeToRational (StakeLedgerGlobal r) = r
  rationalToStake = StakeLedgerGlobal

instance IsStake 'Ledger 'Persistent where
  newtype Stake 'Ledger 'Persistent = StakeLedgerPersistent Rational
    deriving stock (Eq, Ord, Show, Generic)
    deriving newtype Num
  stakeToRational (StakeLedgerPersistent r) = r
  rationalToStake = StakeLedgerPersistent

instance IsStake 'Ledger 'Residual where
  newtype Stake 'Ledger 'Residual = StakeLedgerResidual Rational
    deriving stock (Eq, Ord, Show, Generic)
    deriving newtype Num
  stakeToRational (StakeLedgerResidual r) = r
  rationalToStake = StakeLedgerResidual

instance IsStake 'Ledger 'Cumulated where
  newtype Stake 'Ledger 'Cumulated = StakeLedgerCumulated Rational
    deriving stock (Eq, Ord, Show, Generic)
    deriving newtype Num
  stakeToRational (StakeLedgerCumulated r) = r
  rationalToStake = StakeLedgerCumulated

instance IsStake 'Voting 'Global where
  newtype Stake 'Voting 'Global = StakeVotingGlobal Rational
    deriving stock (Eq, Ord, Show, Generic)
    deriving newtype Num
  stakeToRational (StakeVotingGlobal r) = r
  rationalToStake = StakeVotingGlobal

instance IsStake 'Voting 'Persistent where
  newtype Stake 'Voting 'Persistent = StakeVotingPersistent Rational
    deriving stock (Eq, Ord, Show, Generic)
    deriving newtype Num
  stakeToRational (StakeVotingPersistent r) = r
  rationalToStake = StakeVotingPersistent

instance IsStake 'Voting 'Residual where
  newtype Stake 'Voting 'Residual = StakeVotingResidual Rational
    deriving stock (Eq, Ord, Show, Generic)
    deriving newtype Num
  stakeToRational (StakeVotingResidual r) = r
  rationalToStake = StakeVotingResidual
...
tabulatePersistentToNonPersistentRatio
  numPersistentSeats
  numNonPersistentSeats =
    tabulate "Persistent to non-persistent seat ratio" [show bucket]
   where
    numSeats = numPersistentSeats + numNonPersistentSeats
    -- Divide buckets in 10% incements
    bucket
      | lower == upper = show lower <> "%"
      | otherwise = show lower <> "% ~ " <> show upper <> "%"
     where
      lower = numPersistentSeats * 10 `div` max 1 (numSeats * 10)
                ...
                , -- We still have remaining seats to allocate
                  (seatIndex < numSeats)
                    ...
                    && weightedFaitAccompliThreshold
                      ...
                      -- on global stakes:
                      (coerceStake @Ledger @Global voterStake)
                      stakeAccum
                )
            )
