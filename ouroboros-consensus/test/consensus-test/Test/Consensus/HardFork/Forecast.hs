...
import Data.SOP.InPairs qualified as InPairs
...
advanceTo sno TickedLedgerState{..} =
  TickedLedgerState
...
applyBlock (Block slot blockScheduled) TickedLedgerState{..} =
  LedgerState
    { ledgerScheduled =
        Map.unionWith
          combineUpdates
...
withinEraForecast maxLookAhead st =
  Forecast
...
acrossErasForecast setup@TestSetup{..} ledgerStates =
 ...
 where
  ...
  go (NonEmptyCons era eras) =
    if testEraContains testForecastAt era
      then
        TZ $
          Current
            { currentStart = start
...
prop_forecast useWithinEra (Some setup@TestSetup{..}) =
 ...
 where
  ...
  for :: SlotNo
  for
    | useWithinEra = testForecastWithinEra
...
instance Arbitrary (Some TestSetup) where
  arbitrary = chooseEras $ \ixs -> do
   ...
   where
    ...
    pickSlotBetween lo hi = sized' $ \sz ->
      oneof $
        concat
  ...
  shrink (Some setup@TestSetup{..}) =
   ...
   where
    ...
    shrinkBlock summary maxLookahead mPrev (Block (SlotNo slot) scheduled) =
      concat
    ...
    shrinkScheduled maxLookahead slotBlock =
     ...
     where
      ...
      shrinkUpdate (SlotNo slotUpdate, update@(IncreaseValueBy newLedgerValue)) =
        concat
