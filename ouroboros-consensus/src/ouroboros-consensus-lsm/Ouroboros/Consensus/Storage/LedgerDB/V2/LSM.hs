...
duplicateLSMTable ::
  ...
  UTxOTable m ->
  m (UTxOTable m)
duplicateLSMTable tracer _ t = do
...
loadSnapshot tracer ccfg fs@(SomeHasFS hfs) session ds = do
  ...
  case pointToWithOriginRealPoint (castPoint (getTip extLedgerSt)) of
    ...
    NotOrigin pt -> do
      ...
      h <- lift $ newLSMLedgerTablesHandle tracer msz (getTip extLedgerSt) values
      Monad.when
...
instance
  ...
  createAndPopulateStateRefFromGenesis trcr res st = do
    ...
    StateRef st' <$> newLSMLedgerTablesHandle trcr sz (getTip st) table
