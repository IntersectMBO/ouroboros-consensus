...
instance Typeable ext => EncodeDisk (MockBlock ext) (LedgerState (MockBlock ext) NoTables) where
  encodeDisk _ = encode . simpleLedgerState
instance Typeable ext => DecodeDisk (MockBlock ext) (LedgerState (MockBlock ext) NoTables) where
  decodeDisk _ = flip SimpleLedgerState NoTables <$> decode
