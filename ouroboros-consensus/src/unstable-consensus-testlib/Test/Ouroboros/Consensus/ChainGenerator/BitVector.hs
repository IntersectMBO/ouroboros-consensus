...
findIthEmptyInMV pol mv i =
 ...
 where
  go !j !toSkip =
    if C.getCount (C.lengthMV mv) <= j
      then pure NothingFound
      else do
...
data SomeDensityWindow pol
  = forall slidingWindow.
    SomeDensityWindow
      -- | Numerator: The active slots
      !(C.Var slidingWindow (PreImage pol ActiveSlotE))
      -- | Denominator: The total amount of slots
...
fillInWindow pol (SomeDensityWindow k s) g mv
  ...
  | otherwise = do
      ...
      C.forRange_ (C.toSize adding) $ \alreadyAdded -> do
        ...
        slot <-
          findIthEmptyInMV pol mv whichEmptyToFlip <&> \case
            JustFound i -> i
