...
roundtripAnd check enc dec checkCddlValid a =
  let enc_a = enc a
      ...
      doesRoundtrip = throwLeft $ do
        -- deserialiseFromBytes dec bs `onError` showByteString bs >>= \case
        --   Left e -> undefined
        --   Right (bsRem, a') -> do
    ...
    testProperty "roundtrip Serialised blk" $
      \(WithVersion version (getCoherent -> blk)) ->
        roundtrip @blk
          ...
          blk
  -- TODO(geo2a): this seems to be the only two places we use decodeDepPair.
  --              Do we actually this it in live code?
...
roundtrip_SerialiseNodeToClient shouldCheckCBORvalidity ccfg =
  ...
  , rtWith
      ...
      ( \(queryVersion, blockVersion) query ->
  ...
  , -- See roundtrip_SerialiseNodeToNode for more info
    let testLabel = "roundtrip Serialised blk"
