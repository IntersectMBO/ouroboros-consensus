...
-- | Why an EB closure could not be loaded from the LeiosDb.
...
-- | Reasons we may decline to cast a vote after acquiring an EB closure. See
-- 'TraceLeiosNotVoted'.
...
notVotedReasonText = \case
  ...
  ClosureUnavailable err -> Aeson.String $ "closureUnavailable: " <> fromString (show err)
