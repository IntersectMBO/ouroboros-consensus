...
data LeiosDbHandle m = LeiosDbHandle
  ...
  -- passing the same slot it uses to GC the VolatileDB (see @garbageCollectBlocks@);
  , leiosDbMarkAsImmutable :: HasCallStack => LeiosPoint -> m ()
  -- ^ Mark the given EB as immutable.
  --
  -- The ChainDB's copier (@copyToImmutableDB@) drives this as
  -- it copies blocks to the ImmutableDB.
  --
  -- Keys on the EB /hash/: all of the given EB's announcements are marked, not
  -- just the announced point, because bodies and tx closures are shared across
  -- an EB's announcement slots.
  }
