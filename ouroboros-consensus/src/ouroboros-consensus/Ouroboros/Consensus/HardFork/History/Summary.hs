...
instance Serialise Bound where
  ...
  decode = do
    ...
    boundEpoch <- decode
    boundNextPerasRound <-
      -- NOTE: we have to support legacy serialisation of until we can justify
      -- forcing a replay on all nodes even if Peras is not yet supported.
      -- Defaulting to 0 should be safe for now because Peras rounds only
      -- increase in eras that support it, and until we first enable it, this
