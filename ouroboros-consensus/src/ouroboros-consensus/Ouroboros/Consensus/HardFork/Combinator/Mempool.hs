...
instance CanHardFork xs => TxLimits (HardForkBlock xs) where
  ...
  txMeasure ::
    ...
    Except (ApplyTxErr (HardForkBlock xs)) (TxMeasure (HardForkBlock xs))
  txMeasure
    HardForkLedgerConfig{..}
    (TickedHardForkLedgerState transition hardForkState)
    tx =
     ...
     where
      ...
      aux ::
        ...
        (Product GenTx (Ticked LedgerState m)) blk ->
...
data ApplyResult m xs blk = ApplyResult
  { arState :: Ticked LedgerState m blk
    ...
    case matchPolyTx injs (modeGetTx tx) hardForkState of
      ...
      Right matched ->
        ...
        do
          result <-
            hsequence' $
              hcizipWith proxySingle modeApplyCurrent cfgs (foo matched cache)
          let _ = result :: State.HardForkState (ApplyResult m xs) xs

              st' :: State.HardForkState (Ticked LedgerState m) xs
              st' = arState `hmap` result

              vtx :: Validated (GenTx (HardForkBlock xs))
              vtx = hcollapse $ (K . arValidatedTx) `hmap` result

          return (TickedHardForkLedgerState transition st', vtx, HardForkCache (arCache `hmap` result))
   where
    pcfgs = getPerEraLedgerConfig hardForkLedgerConfigPerEra
    cfgs = hcmap proxySingle (completeLedgerConfig'' ei) pcfgs
    ei =
      ...
      Comp $
        withExceptT (bimap (injectApplyTxErr index) undefined) $
          do
            ...
            case mode of
              ModeApply -> do
                (st', vtx, cache'') <- applyTx lcfg wti slot tx' st cache'
                pure
                  ApplyResult
              ...
              ModeReapply -> do
                ...
                st' <- reapplyTx lcfg slot vtx' st cache'
...
ledgerInfo ::
  forall blk m.
  SingleEraBlock blk =>
  State.Current (Ticked LedgerState m) blk -> LedgerEraInfo blk
