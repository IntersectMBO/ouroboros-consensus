    ...
    where
      ...
      (mismatched, matched) =
        matchPolyTxs
          ...
          (InPairs.hmap product2Snd (InPairs.requiringBoth cfgs hardForkInjectTxs))
          (map (getOneEraValidatedGenTx . getHardForkValidatedGenTx) vtxs)
          hardForkState

      mismatched' :: [Invalidated (HardForkBlock xs)]
      mismatched' =
        map (Invalidated . HardForkValidatedGenTx . OneEraValidatedGenTx . fst) mismatched
      ...
      modeApplyCurrent index cfg (Pair txs (FlipTickedLedgerState st)) =
        ...
        in Comp
           ( map (Invalidated . injectValidatedGenTx index . getInvalidated) err
