...
matchPolyTxTele ::
  ...
  -> Either (Mismatch tx f xs)
            (Telescope g (Product tx f) xs)
matchPolyTxTele ips txs st = go ips txs st
  where
    go :: InPairs (InjectPolyTx tx) xs
       -> NS tx xs
       -> Telescope g f xs
       -> Either (Mismatch tx f xs)
                 (Telescope g (Product tx f) xs)
    go _            (Z x) (TZ f) = Right $ TZ (Pair x f)
    go (PCons _ is) (S x) (TS c f) = bimap MS (TS c) $ go is x f
    go _            (S x) (TZ f) = Left $ MR x f
    go (PCons i is) (Z x) (TS c f) =
        case injectTxWith i x of
          Nothing -> Left $ ML x (Telescope.tip f)
          Just x' -> bimap MS (TS c) $ go is (Z x') f
...
matchPolyTxsTele is ns = go
  where
    ...
    go (tx:txs) =
      ...
      in case matchPolyTxTele is tx matched of
