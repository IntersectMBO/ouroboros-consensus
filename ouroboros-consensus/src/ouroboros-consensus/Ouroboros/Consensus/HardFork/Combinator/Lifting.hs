...
instance
  (All SingleEraBlock xs, forall x. SingleEraBlock x => Eq (f x)) =>
  Eq (LiftNS f xs)
  where
  LiftNS x == LiftNS y =
    case liftEras (Proxy @xs) (Proxy @Eq) (Proxy @f) of
      Dict ->
        x == y

instance
  (All SingleEraBlock xs, forall x. SingleEraBlock x => Ord (f x)) =>
  Ord (LiftNS f xs)
  where
  LiftNS x `compare` LiftNS y =
    case liftEras (Proxy @xs) (Proxy @Eq) (Proxy @f) of
      Dict ->
        case liftEras (Proxy @xs) (Proxy @Ord) (Proxy @f) of
          Dict ->
            x `compare` y

instance
  (All SingleEraBlock xs, forall x. SingleEraBlock x => Show (f x)) =>
  Show (LiftNS f xs)
  where
  show (LiftNS x) =
    case liftEras (Proxy @xs) (Proxy @Show) (Proxy @f) of
      Dict ->
        show x
...
instance
  ...
  Eq (LiftTelescope g f xs)
  where
  LiftTelescope x == LiftTelescope y =
    case liftEras (Proxy @xs) (Proxy @Eq) (Proxy @g) of
      Dict ->
        case liftEras (Proxy @xs) (Proxy @Eq) (Proxy @f) of
          Dict ->
            x == y
...
instance
  ...
  Ord (LiftTelescope g f xs)
  where
  compare (LiftTelescope x) (LiftTelescope y) =
    case liftEras (Proxy @xs) (Proxy @Eq) (Proxy @g) of
      Dict ->
        case liftEras (Proxy @xs) (Proxy @Ord) (Proxy @g) of
          Dict ->
            case liftEras (Proxy @xs) (Proxy @Eq) (Proxy @f) of
              Dict ->
                case liftEras (Proxy @xs) (Proxy @Ord) (Proxy @f) of
...
instance
  ...
  Show (LiftTelescope g f xs)
  where
  show (LiftTelescope x) =
    case liftEras (Proxy @xs) (Proxy @Show) (Proxy @g) of
      Dict ->
        case liftEras (Proxy @xs) (Proxy @Show) (Proxy @f) of
          Dict ->
            show x
...
instance
  ...
  NoThunks (LiftNamedNP name f xs)
  where
  showTypeOf _ = symbolVal (Proxy @name) ++ " " ++ showBlockTypes (sList :: SList xs)
...
instance
  ...
  NoThunks (LiftNamedTelescope name f g xs)
  where
  showTypeOf _ = symbolVal (Proxy @name) ++ " " ++ showBlockTypes (sList :: SList xs)
  ...
  wNoThunks ctxt (LiftNamedTelescope x) =
    case liftEras (Proxy @xs) (Proxy @NoThunks) (Proxy @f) of
      Dict ->
        case liftEras (Proxy @xs) (Proxy @NoThunks) (Proxy @g) of
          Dict ->
            wNoThunks ctxt x
...
instance
  ...
  NoThunks (LiftNamedMismatch name f g xs)
  where
  showTypeOf _ = symbolVal (Proxy @name) ++ " " ++ showBlockTypes (sList :: SList xs)
