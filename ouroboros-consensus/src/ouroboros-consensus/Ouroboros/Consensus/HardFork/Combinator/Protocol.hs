...
instance CanHardFork xs => AChainDepSupportsPeras (HardForkChainDepState xs) where
  getEpochNonce =
    hcollapse
...
instance CanHardFork xs => AChainDepSupportsPeras (Ticked (HardForkChainDepState xs)) where
  getEpochNonce =
    hcollapse
      . hcmap proxySingle getEpochNonce'
      . State.tip
