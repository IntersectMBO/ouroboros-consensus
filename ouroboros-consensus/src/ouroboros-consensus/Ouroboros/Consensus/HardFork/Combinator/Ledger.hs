...
-- | 'ALedgerSupportsPeras' for the /ticked/ hard fork ledger state, mirroring
-- the instance for the unticked ledger state in
-- "Ouroboros.Consensus.HardFork.Combinator.Basics".
instance
  CanHardFork xs =>
  ALedgerSupportsPeras (Ticked LedgerState (HardForkBlock xs) mk)
  where
  getLatestPerasCertRound =
    hcollapse
      . hcmap proxySingle (K . getLatestPerasCertRound . getFlipTickedLedgerState)
      . State.tip
      . tickedHardForkLedgerStatePerEra
...
-- 'ALedgerSupportsPeras' instance above) are not in scope in
