...
answerBlockQueryHelper ::
  ...
  ( NP ExtLedgerCfg xs ->
    QueryIfCurrent xs footprint result ->
    NS (StateRef m ExtLedgerState) xs ->
    m (HardForkQueryResult xs result)
  ) ->
  ExtLedgerCfg (HardForkBlock xs) ->
  QueryIfCurrent xs footprint result ->
  State.HardForkState (StateRef m ExtLedgerState) xs ->
  m (HardForkQueryResult xs result)
answerBlockQueryHelper
  f
  (ExtLedgerCfg cfg)
...
hfExtLedgerState ::
  ...
  StateRef m ExtLedgerState (HardForkBlock xs) -> State.HardForkState (StateRef m ExtLedgerState) xs
hfExtLedgerState (ExtStateRef (HardForkStateRef (State.HardForkState st)) headerState) =
  case matchTelescope (distribHeaderState headerState) st of
    Left _err -> error "impossible!"
    Right tele ->
      State.HardForkState $
        hcmap proxySingle (\(Pair hs (Current c ls)) -> Current c (ExtStateRef ls hs)) tele
