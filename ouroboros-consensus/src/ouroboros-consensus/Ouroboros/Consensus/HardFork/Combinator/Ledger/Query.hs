...
answerBlockQueryHelper ::
  ...
  ( NP ExtLedgerCfg xs ->
    QueryIfCurrent xs footprint result ->
    NS (ExtLedgerState m) xs ->
    m (HardForkQueryResult xs result)
  ) ->
  ExtLedgerCfg (HardForkBlock xs) ->
  QueryIfCurrent xs footprint result ->
  State.HardForkState (ExtLedgerState m) xs ->
  m (HardForkQueryResult xs result)
answerBlockQueryHelper
  f
  (ExtLedgerCfg cfg)
  ...
  forker = do
    let hardForkState = State.getHardForkState forker
        ei = State.epochInfoLedger lcfg $ hmap ledgerState forker
        cfgs = hmap ExtLedgerCfg $ distribTopLevelConfig ei cfg
...
distribExtLedgerState ::
  All SingleEraBlock xs =>
  ExtLedgerState m (HardForkBlock xs) -> NS (ExtLedgerState m) xs
...
hfExtLedgerState ::
  ExtLedgerState m (HardForkBlock xs) -> State.HardForkState (ExtLedgerState m) xs
hfExtLedgerState = undefined
...
interpretQueryAnytime ::
  ...
  QueryAnytime result ->
  EraIndex xs ->
  State.HardForkState (LedgerState m) xs ->
  result
