...
import Ouroboros.Consensus.Peras.Weight (WeightedSelectView (..), WithEmptyFragment (..))
...
selectChain _ cfg view ours =
 ...
 where
  ...
  selectPreferredCandidate ::
    (Chain hdr, l) ->
    Maybe (WeightedSelectView p, (Chain hdr, l))
  selectPreferredCandidate x@(cand, _) =
    case (view ours, view cand) of
      (EmptyFragment, NonEmptyFragment candView) ->
        Just (candView, x)
      (NonEmptyFragment ourView, NonEmptyFragment candView)
        | preferCandidate cfg ourView candView ->
            Just (candView, x)
      _otherwise ->
        Nothing
