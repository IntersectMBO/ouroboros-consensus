...
runAgainstBucket config action = do
  ...
  where
    startThread :: StrictTVar m (Maybe (m ())) -> Bucket m -> ThreadId m -> m ()
    startThread killThreadVar bucket tid =
    ...
    updateConfig :: StrictTVar m (Maybe (m ())) -> Bucket m -> ThreadId m -> (Config m -> Config m) -> m ()
...
init :: (MonadMonotonicTime m, MonadSTM m, NoThunks (m ())) => Config m -> m (Bucket m)
...
leak :: (MonadDelay m, MonadCatch m, MonadFork m, MonadAsync m) => Bucket m -> ThreadId m -> m (Maybe (m ()))
