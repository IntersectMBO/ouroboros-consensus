...
-- | Hash the message to be signed for a Peras vote
hashVoteSignature ::
  forall blk.
  ConvertRawHash blk =>
  PerasRoundNo ->
...
hashVoteSignature roundNo point =
 ...
 where
  ...
  pointBytes =
    case point of
      GenesisPoint -> mempty
      BlockPoint _ h -> BS.byteStringCopy (toRawHash (Proxy @blk) h)
...
instance ConvertRawHash blk => CryptoSupportsVoteSigning (PerasVoteCrypto blk) where
  type VoteSignaturePrivateKey (PerasVoteCrypto blk) = PerasBLSPrivateKey SIGN
  type VoteSignaturePublicKey (PerasVoteCrypto blk) = PerasBLSPublicKey SIGN

  newtype VoteMessage (PerasVoteCrypto blk)
    = PerasVoteMessage (Point blk)
    deriving (Eq, Show)

  newtype VoteSignature (PerasVoteCrypto blk)
    = PerasVoteSignature (PerasSignature SIGN)
    deriving (Eq, Show)

  signVote sk roundNo (PerasVoteMessage point) =
    PerasVoteSignature $
      perasSignWithRole @SIGN sk $
        hashVoteSignature roundNo point

  verifyVoteSignature pk roundNo (PerasVoteMessage point) (PerasVoteSignature sig) =
    perasVerifyWithRole @SIGN
      pk
      (hashVoteSignature roundNo point)
      sig
