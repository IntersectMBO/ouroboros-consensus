...
-- | Tie concrete Peras votes to their crypto scheme
type family PerasVoteCrypto vote :: Type

-- | Tie concrete Peras certificates to their crypto scheme
type family PerasCertCrypto cert :: Type

-- Version 1 'PerasVote's and 'PerasCert's using 'PerasBLSCrypto'
type instance PerasVoteCrypto V1.PerasVote = PerasBLSCrypto
type instance PerasCertCrypto V1.PerasCert = PerasBLSCrypto
...
-- | Voting committees a concrete Peras vote can be used with
class PerasVoteSupportsVotingCommittee vote committee where
  toPerasVote ::
    Committee.Vote (PerasVoteCrypto vote) committee ->
    Either PerasConversionError vote
  fromPerasVote ::
    vote ->
    Either PerasConversionError (Committee.Vote (PerasVoteCrypto vote) committee)

-- | Voting committees a concrete Peras certificate can be used with
class PerasCertSupportsVotingCommittee cert committee where
  toPerasCert ::
    Committee.Cert (PerasCertCrypto cert) committee ->
    Either PerasConversionError cert
  fromPerasCert ::
    cert ->
    Either PerasConversionError (Committee.Cert (PerasCertCrypto cert) committee)

-- 'V1.PerasVote's are compatible with 'WFALS' as long as we make sure to avoid
-- overflowing their `Word16` seat index.
instance PerasVoteSupportsVotingCommittee V1.PerasVote WFALS where
  toPerasVote = \case
...
instance PerasCertSupportsVotingCommittee V1.PerasCert WFALS where
  toPerasCert = \case
...
instance PerasVoteSupportsVotingCommittee V1.PerasVote EveryoneVotes where
  toPerasVote = \case
...
instance PerasCertSupportsVotingCommittee V1.PerasCert EveryoneVotes where
  toPerasCert = \case
