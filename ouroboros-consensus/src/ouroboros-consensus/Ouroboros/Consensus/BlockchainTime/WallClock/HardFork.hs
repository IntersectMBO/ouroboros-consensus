...
data HardForkBlockchainTimeArgs m blk = HardForkBlockchainTimeArgs
  ...
  -- ^ See 'BackoffDelay'
  , hfbtGetLedgerState :: STM m (LedgerState blk NoTables)
...
hardForkBlockchainTime args = do
  ...
  void $
    forkLinkedThread registry "hardForkBlockchainTime" $
 ...
 where
  ...
  summarize :: LedgerState blk NoTables -> HF.Summary (HardForkIndices blk)
    ...
    case (prevSlot, newSlot) of
      ...
      (CurrentSlot m, CurrentSlot n)
        ...
        , prevTime `diffRelTime` newTime <= maxClockRewind ->
            do
              traceWith tracer $ TraceSystemClockMovedBack prevTime newTime
              return prevSlot
        | otherwise ->
