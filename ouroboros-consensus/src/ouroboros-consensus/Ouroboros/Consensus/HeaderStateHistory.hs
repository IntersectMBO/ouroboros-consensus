...
cast (HeaderStateHistory history) =
    HeaderStateHistory $ join AS.bimap castHeaderStateWithTime history
...
rewind p (HeaderStateHistory history) = do
    (prefix, suffix) <- AS.splitAfterMeasure
      (pointSlot p)
      ((== p) . join either (headerStatePoint . hswtHeaderState))
      history
...
mkHeaderStateWithTimeFromSummary ::
     forall blk. (HasCallStack, HasAnnTip blk)
...
mkHeaderStateWithTime ::
     forall blk. (HasCallStack, HasHardForkHistory blk, HasAnnTip blk)
  => LedgerConfig blk
  -> ExtLedgerState blk
  -> HeaderStateWithTime blk
