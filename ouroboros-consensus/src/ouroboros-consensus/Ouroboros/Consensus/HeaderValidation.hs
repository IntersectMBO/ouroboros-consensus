...
class
  ...
  default getTipInfo ::
    (TipInfo blk ~ HeaderHash blk, HasHeader (Header blk)) =>
...
validateIfCheckpoint checkpointsMap hdr =
  whenJust (Map.lookup (blockNo hdr) $ unCheckpointsMap checkpointsMap) $
    \checkpoint ->
      when (headerHash hdr /= checkpoint) $
        throwError $
          CheckpointMismatch (blockNo hdr) checkpoint (headerHash hdr)
...
castHeaderError (HeaderEnvelopeError e) =
  HeaderEnvelopeError $
...
defaultEncodeAnnTip ::
  ...
  (AnnTip blk -> Encoding)
defaultEncodeAnnTip encodeHash AnnTip{..} =
  mconcat
    [ encodeListLen 3
    , encode annTipSlotNo
...
encodeAnnTipIsEBB ::
  ...
  (HeaderHash blk -> Encoding) ->
  (AnnTip blk -> Encoding)
encodeAnnTipIsEBB encodeHash AnnTip{..} =
  mconcat
    [ encodeListLen 4
    , encode annTipSlotNo
    , encodeHash hash
