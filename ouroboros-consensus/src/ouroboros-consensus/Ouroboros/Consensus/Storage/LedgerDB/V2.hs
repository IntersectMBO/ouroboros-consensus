...
mkInitDb args getBlock snapManager getVolatileSuffix res = do
  InitDB
    { initFromGenesis = do
        st <- lift $ lgrGenesis
        ref@(StateRef _ h) <- newHandleFromValues v2Tracer res st
        pure
          ( do
              -- We untrack here because this will definitely go into the LedgerDB
              atomically (untrack h)
              pure (LedgerSeq (AS.Empty ref))
          )
    , initFromSnapshot = \snap -> do
        eRef <-
          tryContT $
            newHandleFromSnapshot
              ...
              snap
        case eRef of
          Right (ref@(StateRef _ h), pt) ->
            pure
              ( do
                  -- We untrack here because this will definitely go into the LedgerDB
                  atomically (untrack h)
                  pure (Right (LedgerSeq (AS.Empty ref), pt))
              )
          Left err -> pure (pure (Left err))
    , initReapplyBlock = \a b c -> do
        (doUntrack, doClose, db') <- reapplyThenPush a b c
        lift doClose
        -- We untrack here because this will definitely go into the LedgerDB
        pure (atomically doUntrack >> pure db')
...
implMkLedgerDb h snapManager =
  ( LedgerDB
      ...
      , getForkerForReadingAtTarget = newForkerForReadingAtTarget h
      , getForkerForMempoolAtTarget = newForkerForMempoolAtTarget h
...
mkInternals h snapManager =
  TestInternals
    ...
    , push = \st -> do
        newChainSelectionForkerByRollback h 0 `runContT` \case
          Left{} -> error "Unreachable, Volatile tip MUST be in LedgerDB"
          Right frk -> do
            forkerPush' frk st `runContT` \() -> do
              atomically (forkerCommit' frk)
              getEnv h pruneLedgerSeq
    , reapplyThenPushNOW = \blk -> getEnv h $ \env ->
        newChainSelectionForkerByRollback h 0 `runContT` \case
          Left{} -> error "Unreachable, Volatile tip MUST be in LedgerDB"
          Right frk -> do
            st <- atomically $ forkerGetLedgerState' (ffcsReadForker frk)
            tables <- forkerReadTables' (ffcsReadForker frk) (getBlockKeySets blk)
            let st' =
                  tickThenReapply
...
implValidate h ldbEnv tr cache rollbacks hdrs =
  validate (ledgerDbCfgComputeLedgerEvents $ ldbCfg ldbEnv) $
    ValidateArgs
      ...
      (newChainSelectionForkerByRollback h)
      tr
      cache
      rollbacks
      hdrs
...
getEnvCont ::
  ...
  (IOLike m, HasCallStack, HasHeader blk) =>
  LedgerDBHandle m l blk ->
  (LedgerDBEnv m l blk -> ContT r m a) ->
  ContT r m a
getEnvCont (LDBHandle varState) f =
  lift (readTVarIO varState) >>= \case
    LedgerDBOpen env -> f env
    LedgerDBClosed -> lift $ throwIO $ ClosedDBError @blk prettyCallStack
...
withStateRef ldbEnv project f =
  getStateRef ldbEnv project `runContT` f
...
acquireAtTarget ::
  ...
  ContT r m (Either GetForkerError (StateRef m l))
...
newForkerForReadingAtTarget h pt = getEnvCont h $ \ldbEnv ->
  acquireAtTarget ldbEnv (Right pt) >>= traverse (newForkerForReading h ldbEnv)

newForkerForMempoolAtTarget ::
  ( HeaderHash l ~ HeaderHash blk
  , IOLike m
  , IsLedger l
  , LedgerSupportsProtocol blk
...
newForkerForMempoolAtTarget h pt = getEnvCont h $ \ldbEnv ->
  acquireAtTarget ldbEnv (Right pt) >>= traverse (newForkerForMempool h ldbEnv)

newChainSelectionForkerByRollback ::
  ( HeaderHash l ~ HeaderHash blk
  , IOLike m
  , IsLedger l
  , StandardHash l
  ...
  LedgerDBHandle m l blk ->
  Word64 ->
  ContT r m (Either GetForkerError (ForkerForChainSelection m l))
newChainSelectionForkerByRollback h n = getEnvCont h $ \ldbEnv ->
  acquireAtTarget ldbEnv (Left n) >>= traverse (newForkerForChainSelection h ldbEnv)
...
getForkerEnvCont ::
  ...
  LedgerDBHandle m l blk ->
  ForkerKey ->
  (ForkerEnv m l blk -> ContT r m a) ->
  ContT r m a
getForkerEnvCont (LDBHandle varState) forkerKey f = do
  forkerEnv <-
    lift $
      atomically $
        readTVar varState >>= \case
...
getForkerEnvCont1 ::
  ...
  LedgerDBHandle m l blk ->
  ForkerKey ->
  (ForkerEnv m l blk -> a -> ContT r m b) ->
  a ->
  ContT r m b
getForkerEnvCont1 h forkerKey f a = getForkerEnvCont h forkerKey (`f` a)
...
newForkerForMempool ::
  ...
  LedgerDBEnv m l blk ->
  StateRef m l ->
  ContT r m (ForkerForMempool m l)
newForkerForMempool h ldbEnv st = ContT $ \k -> do
  bracket
    ( do
        forkerKey <- atomically $ stateTVar (ldbNextForkerKey ldbEnv) $ \r -> (r, r + 1)
        let tr = LedgerDBForkerEvent . TraceForkerEventWithKey forkerKey >$< ldbTracer ldbEnv
        ...
        pure
          ...
          , ForkerForMempool
              ...
              , forkerRelease = implForkerClose h forkerKey forkerEnv
              }
          )
    )
    ( \(lseqVar, _) -> do
...
newForkerForReading ::
  ...
  LedgerDBEnv m l blk ->
  StateRef m l ->
  ContT r m (ForkerForReading m l)
newForkerForReading h ldbEnv st = ContT $ \k -> do
  bracket
    ( do
        forkerKey <- atomically $ stateTVar (ldbNextForkerKey ldbEnv) $ \r -> (r, r + 1)
        let tr = LedgerDBForkerEvent . TraceForkerEventWithKey forkerKey >$< ldbTracer ldbEnv
    ...
    (\(forkerKey, forkerEnv, _) -> implForkerClose h forkerKey forkerEnv)
    (\(_, _, forker) -> k forker)

newForkerForChainSelection ::
  ( IOLike m
  , HasLedgerTables l
  , LedgerSupportsProtocol blk
  , NoThunks (l EmptyMK)
  ...
  LedgerDBEnv m l blk ->
  StateRef m l ->
  ContT r m (ForkerForChainSelection m l)
newForkerForChainSelection h ldbEnv st = ContT $ \k -> do
  bracket
    ( do
        forkerKey <- atomically $ stateTVar (ldbNextForkerKey ldbEnv) $ \r -> (r, r + 1)
        let tr = LedgerDBForkerEvent . TraceForkerEventWithKey forkerKey >$< ldbTracer ldbEnv
