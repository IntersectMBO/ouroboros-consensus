...
import Control.Monad qualified as Monad (void, (>=>))
...
import Ouroboros.Consensus.Storage.LedgerDB.V2.InMemory qualified as InMemory
    ...
    { takeSnapshotNOW = \whereTo suff -> getEnv h $ \env -> do
        st <-
          ( case whereTo of
              ...
              TakeAtImmutableTip -> currentHandle
          )
