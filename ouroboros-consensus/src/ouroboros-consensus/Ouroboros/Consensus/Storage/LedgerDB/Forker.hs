...
-- | Forker-level events.
--
-- Per-handle events (reads, range reads, pushes, statistics) live on the
-- backend tracer ('LedgerDBV2Trace') instead, because reads and pushes now
-- happen against an 'ExtStateHandle' rather than against a 'Forker'.
