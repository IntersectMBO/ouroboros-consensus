...
-- responsible for:
--
-- - Maintaining the last \(k\) in-memory ledger states.
--
...
--
-- When a new ledger state is appended to a fully saturated 'DbChangelog', the
-- ledger state at the anchor is dropped and the oldest element in the sequence
-- becomes the new anchor, as it has become immutable. This maintains the
-- invariant that only the last \(k\) in-memory ledger states are stored,
-- /excluding/ the ledger state at the anchor. This means that in practice, \(k
-- + 1\) ledger states will be kept in memory. When the 'DbChangelog' contains
-- fewer than \(k\) elements, new ones are appended without shifting the anchor
