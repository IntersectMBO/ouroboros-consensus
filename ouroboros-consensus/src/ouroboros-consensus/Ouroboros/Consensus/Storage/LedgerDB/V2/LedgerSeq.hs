...
reapplyThenPush rr cfg ap db =
    (\current' -> prune (LedgerDBPruneTip (ledgerDbCfgSecParam cfg)) $ extend current' db) <$>
...
prune (LedgerDBPruneTip (SecurityParam k)) (LedgerSeq ldb) =
  ...
  where
    nvol = AS.length ldb
prune LedgerDBPruneTipZero (LedgerSeq ldb) =
...
pruneToImmTipOnly :: GetTip l
                  => LedgerSeq m l
                  -> (LedgerSeq m l, LedgerSeq m l)
pruneToImmTipOnly = prune LedgerDBPruneTipZero
