...
-- data LedgerTablesHandle m l v = LedgerTablesHandle
--   { read :: Set SL.TxIn -> m v
--   , duplicateWithDiffs :: Diff.Diff SL.TxIn v -> m (Handle m v)
--   , duplicate :: m (Handle m v)
--   , readRange :: RangeQueryPrevious -> m (v, Maybe SL.TxIn)
--   , applyDifferences :: Diff.Diff SL.TxIn v -> m ()
--   , takeHandleSnapshot :: l -> String -> m ()
--   , close :: m ()
--   , tablesSize :: Int
--   }
...
-- newInMemoryLedgerTablesHandle !tracer !someFS@(SomeHasFS !hasFS)  =
