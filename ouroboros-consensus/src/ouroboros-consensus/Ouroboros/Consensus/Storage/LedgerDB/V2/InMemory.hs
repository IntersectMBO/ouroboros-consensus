...
newInMemoryLedgerTablesHandle !tracer !someFS@(SomeHasFS !hasFS) (origResKey, tv) =
    ...
    NotOrigin pt -> do
      ...
      (rk, values') <-
        lift $
          allocate
            rr
            (\_ -> newTVarIO $ LedgerTablesHandleOpen values)
            ( \tv' -> do
                p <- atomically $ swapTVar tv' LedgerTablesHandleClosed
                case p of
                  LedgerTablesHandleOpen{} -> encloseTimedWith (TraceLedgerTablesHandleClose >$< tracer) $ pure ()
...
instance
  ...
  newHandleFromValues tracer rr (Resources shfs) values = do
    (rk, values') <-
      allocate
        rr
        (\_ -> newTVarIO (LedgerTablesHandleOpen $ ltprj values))
        ( \tv' -> do
            p <- atomically $ swapTVar tv' LedgerTablesHandleClosed
            case p of
              LedgerTablesHandleOpen{} -> encloseTimedWith (TraceLedgerTablesHandleClose >$< tracer) $ pure ()
