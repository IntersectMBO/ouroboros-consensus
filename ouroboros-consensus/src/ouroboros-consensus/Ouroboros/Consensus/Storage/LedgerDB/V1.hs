...
unsafeNewForkerAtTarget h pt = withTransferrableReadAccess h (Right pt)
...
implWithForkerAtTip ::
  ...
  (Forker m l -> m r) ->
  m r
implWithForkerAtTip h k =
  bracket
    (withTransferrableReadAccess h (Right VolatileTip))
    (either (const $ pure ()) forkerClose)
