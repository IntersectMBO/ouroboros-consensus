...
withTipForker ::
  ...
  (Forker m l blk -> m a) ->
  m a
withTipForker ldb label rr =
  bracket
    ( do
        eFrk <- getForkerAtTarget ldb label rr VolatileTip
        case eFrk of
...
withPrivateTipForker ::
  ...
  (Forker m l blk -> m a) ->
  m a
withPrivateTipForker ldb label =
...
getTipStatistics ::
  ...
  LedgerDB m l blk ->
  m (Maybe Statistics)
getTipStatistics ldb = withPrivateTipForker ldb "getTipStatistics" forkerReadStatistics
...
getReadOnlyForker ldb label rr pt = fmap readOnlyForker <$> getForkerAtTarget ldb label rr pt
