  ...
  where
    ...
    constructChains i succsOf = flip evalStateT Map.empty $
      ...
      where
        ...
        suffixesAfterI = Paths.maximalCandidates succsOf k loeFrag curChain (AF.anchorToPoint i)
          where
            curChain = Empty (AF.castAnchor i)
            loeFrag = curChain <$ loE
...
chainSelectionForBlock cdb@CDB{..} blockCache hdr punish = electric $ do
  ...
  where
    ...
    addToCurrentChain succsOf curChainAndLedger loeFrag = do
        ...
        let suffixesAfterB =
              Paths.maximalCandidates succsOf k loeFrag (curChain AF.:> hdr) (realPointToPoint p)
    ...
    switchToAFork succsOf lookupBlockInfo curChainAndLedger loeFrag diff = do
        ...
        chainDiffs <-
          ...
          . Paths.extendWithSuccessors succsOf lookupBlockInfo k loeFrag curChain
          $ diff
