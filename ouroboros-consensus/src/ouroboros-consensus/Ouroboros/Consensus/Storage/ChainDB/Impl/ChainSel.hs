...
initialChainSelection
  ...
  weights = do
    ...
    case NE.nonEmpty
      ...
      Just chains' ->
        maybe curChain (getSuffix . fst) <$> chainSelection' curChain chains'
   where
  ...
  go ((candidate, reason) : candidates0) = do
    case NE.nonEmpty (AF.toOldestFirst $ getSuffix candidate) of
      ...
      Just neBlocks -> do
        ...
        validateCandidate chainSelEnv candidate neBlocks (onSuccess candidate reason) >>= \case
          FullyValid candidate' -> do
...
validateCandidate chainSelEnv chainDiff@(ChainDiff rollback suffix) neBlocks onSuccess =
