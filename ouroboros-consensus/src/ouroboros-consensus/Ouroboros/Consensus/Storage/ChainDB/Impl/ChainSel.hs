        ...
        assert (all (shouldSwitch . preferAnchoredCandidate bcfg weights curChain . fst) candidates) $ do
          ...
          fmap (getSuffix . fst)
            <$> chainSelection
              ...
              (\_ _ -> join . atomically . forkerCommit)
     where
...
chainSelection ::
  ...
  (ChainDiff (Header blk) -> ReasonForSwitch' blk -> Forker ExtLedgerState m blk -> m ()) ->
