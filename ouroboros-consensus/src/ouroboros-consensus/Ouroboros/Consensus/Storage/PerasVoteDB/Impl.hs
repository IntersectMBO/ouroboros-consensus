...
implAddVote
  ...
  vote = do
    ...
    res <- atomically $ do
      ...
      if Set.member voteId pvvsVoteIds
        then pure PerasVoteAlreadyInDB
        else do
          let pvsVoteIds' = Set.insert voteId pvvsVoteIds
              pvsLastTicketNo' = succ pvvsLastTicketNo
              pvsVotesByTicket' = Map.insert pvsLastTicketNo' vote pvvsVotesByTicket
              fp' = succ fp

          (res, pvsRoundVoteStates') <-
            case updatePerasRoundVoteStates vote perasCfg pvvsRoundVoteStates of
