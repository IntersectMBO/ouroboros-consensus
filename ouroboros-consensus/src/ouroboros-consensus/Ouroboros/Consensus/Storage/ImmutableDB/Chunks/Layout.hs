...
chunkIndexOfSlot (UniformChunkSize ChunkSize{..}) (SlotNo slot) =
  ChunkNo $
...
chunkSlotForBoundaryBlock ci epoch =
  assertChunkCanContainEBB chunk size $
    UnsafeChunkSlot chunk $
      firstBlockOrEBB ci chunk
 where
...
chunkSlotForTip ci Tip{tipSlotNo, tipIsEBB} = case tipIsEBB of
  ...
  IsEBB ->
    assertChunkCanContainEBB chunkIndex relativeSlotChunkSize $
      UnsafeChunkSlot chunkIndex $
        firstBlockOrEBB ci chunkIndex
 where
...
chunkSlotToSlot (UniformChunkSize ChunkSize{..}) UnsafeChunkSlot{..} =
  SlotNo $
