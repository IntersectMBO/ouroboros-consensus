...
instance
  ...
  Exception (ImmutableDBError blk)
  where
...
data MissingBlock blk
  = -- | There is no block in the slot of the given point.
    EmptySlot
      -- | The requested point
      (RealPoint blk)
      -- | The chunk we thought it was in
      ChunkNo
      -- | What should be the relative slot in the chunk
      [RelativeSlot]
...
streamAfterPoint db registry blockComponent fromPt = runExceptT $ do
  ...
  case ( pointToWithOriginRealPoint fromPt
       , pointToWithOriginRealPoint tipPt
       ) of
    (Origin, Origin) ->
      -- Nothing to stream
    ...
    (NotOrigin fromPt', NotOrigin _)
      | pointSlot fromPt > pointSlot tipPt ->
          -- Lower bound is newer than the tip, nothing to stream
          throwError $ NewerThanTip fromPt' tipPt
    (NotOrigin fromPt', NotOrigin tipPt')
      | fromPt' == tipPt' ->
