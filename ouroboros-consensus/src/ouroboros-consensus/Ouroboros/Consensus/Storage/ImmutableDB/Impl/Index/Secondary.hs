...
readEntry hasFS chunk isEBB slotOffset =
  runIdentity
    ...
    forM toRead $ \(isEBB, slotOffset) -> do
      ...
      if anotherEntryAfter
        then do
          ...
          entry <-
            hGetExactlyAt hasFS sHnd nbBytes offset
...
readAllEntries ::
  ...
  (Entry blk -> Bool) ->
  -- | The size of the chunk file, used to compute
...
readAllEntries hasFS secondaryOffset chunk stopAfter chunkFileSize = \isEBB ->
 ...
 where
  ...
  go ::
    IsEBB ->
    -- \^ Interpret the next entry as an EBB?
    Lazy.ByteString ->
  ...
  go isEBB bl acc mbPrevEntry
    ...
    | otherwise = do
        ...
        let offsetAfterPrevBlock = unBlockOffset (blockOffset entry)
            acc' =
              (addBlockSize offsetAfterPrevBlock <$> mbPrevEntry)
                `consMaybe` acc
        if stopAfter entry
          then
