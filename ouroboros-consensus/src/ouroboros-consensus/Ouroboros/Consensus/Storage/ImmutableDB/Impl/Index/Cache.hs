...
import Ouroboros.Consensus.Storage.ImmutableDB.Impl.Index.Primary qualified as Primary
import Ouroboros.Consensus.Storage.ImmutableDB.Impl.Index.Secondary
  ( BlockSize (..)
  )
import Ouroboros.Consensus.Storage.ImmutableDB.Impl.Index.Secondary qualified as Secondary
...
emptyCached ::
  ...
  CurrentChunkInfo blk ->
  Cached blk
emptyCached currentChunk currentChunkInfo =
  Cached
    { currentChunk
    , currentChunkInfo
...
newEnv hasFS registry tracer cacheConfig chunkInfo chunk = do
  ...
  mask_ $ modifyMVar_ bgThreadVar $ \_mustBeNothing -> do
    !bgThread <-
      forkLinkedThread registry "ImmutableDB.expireUnusedChunks" $
...
getChunkInfo cacheEnv chunk = do
  ...
  (mbCacheHit, tr) <- modifyMVar cacheVar $
    \cached@Cached{currentChunk, currentChunkInfo, nbPastChunks} ->
      if
...
readOffsets cacheEnv chunk relSlots = do
 ...
 where
  ...
  getOffsetFromSecondaryOffsets offsets relSlot =
    let s = assertRelativeSlotInChunk chunk relSlot
     in case Seq.splitAt (fromIntegral s + 1) offsets of
...
appendOffsets CacheEnv{hasFS, cacheVar} pHnd offsets = do
 ...
 where
  ...
  addCurrentChunkOffsets cached@Cached{currentChunkInfo} =
    cached
      { currentChunkInfo =
          currentChunkInfo
            { currentChunkOffsets =
                currentChunkOffsets currentChunkInfo
...
readEntries cacheEnv chunk toRead =
 ...
 where
  ...
  indexForOffset secondaryOffset =
    fromIntegral $
    ...
    getChunkInfo cacheEnv chunk <&> \case
      Left CurrentChunkInfo{currentChunkEntries} ->
        takeUntil (stopCondition . withoutBlockSize) $
          toList $
            Seq.drop toDrop currentChunkEntries
      Right PastChunkInfo{pastChunkEntries} ->
        takeUntil (stopCondition . withoutBlockSize) $
          toList $
            Vector.drop toDrop pastChunkEntries
   where
    toDrop :: Int
    toDrop =
      fromIntegral $
