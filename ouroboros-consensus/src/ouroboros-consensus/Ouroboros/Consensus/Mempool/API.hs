...
data MempoolSnapshot blk = MempoolSnapshot
  ...
  , snapshotTakeWithInitialPayload ::
      Maybe (TxMeasureWithDiffTime blk) -> TxMeasure blk -> ([Validated (GenTx blk)], TxMeasureWithDiffTime blk)
  -- ^ Get the greatest prefix (oldest to newest) that respects the given
  -- block capacity, an optional initial payload in the block, and the prefix's total size.
