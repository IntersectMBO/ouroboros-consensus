...
data MempoolSnapshot blk = MempoolSnapshot
  ...
  , snapshotTakeWithInitialPayload ::
      TxMeasureWithDiffTime blk -> TxMeasure blk -> ([Validated (GenTx blk)], TxMeasureWithDiffTime blk)
  -- ^ Get the greatest prefix (oldest to newest) that respects the given
  -- block capacity, the initial payload of the block, and the prefix's total size.
