    ...
    doAddTx' mbPrevSize = do
          ...
          case mTbs of
            ...
            Just tbs -> do
              ...
              mbX <- do
                ...
                f $ do
                  x <- evaluate $ pureTryAddTx mbToCfg cfg wti tx is tbs
                  case (caller, x) of
                    ...
                    -- Note that @sofar == 0@ always and this 'threadDelay'
                    -- would be perfectly precise in the @IOSim@ monad.
                    -- Unfortunately, the state machines tests are still only
                    -- in IO.
                    _ -> pure ()
                  pure x
              dur <- do
                -- Note that both the hard 'timeout' and the soft duration
                -- check use the actual monotonic clock measurements instead of
                -- simply deferring to 'TestingAddTx'. This means the test will
                -- fail if the 'timeout' and the monotonic clock measurement
                -- primitives are not as precise as the test expects (recall
                -- that the test suite chooses intended validation times that
                -- are not "too close" to the thresholds).
                after <- getMonotonicTime
              ...
              case mbX of
                ...
                Just _
                  ...
                  , Just txerr <- mkMempoolPredicateFailure (isLedgerState is) txt ->
                      -- The txerr is not available in historical Cardano eras,
                      -- but it is starting from Conway. So this rejection will
                      -- be disabled prior to Conway. Which is irrelevant,
                      -- since mainnet is already in Conway.
                      do
                        let outcome =
                              TransactionProcessingResult
                                Nothing
                ...
                Just (Processed mkResult) -> do
                  let outcome = mkResult $ FiniteDiffTimeMeasure $ case caller of
                        ...
                        TestingAddTx testDiffTime ->
                          -- For the sake of an accurate cumulative measure,
                          -- pretend the tx took exactly as long to validate as
                          -- the test suite intended.
                          --
                          -- Note that @testDiffTime == dur@ always in @IOSim@.
                          -- Unfortunately, the state machines tests are still
                          -- only in IO.
                          testDiffTime
      ...
      case (caller, res) of
        (_, Resync) -> do
          ...
          doAddTx' mbPrevSize
        (ProductionAddTx, Retry s') -> doAddTx' (Just s')
        (ProductionAddTx, OK outcome) -> pure $ Identity outcome
        (TestingAddTx _, Retry _) -> pure Nothing
        (TestingAddTx _, OK x) -> pure $ Just x
...
pureTryAddTx ::
  ...
  -> TriedToAddTx blk
pureTryAddTx mbToCfg cfg wti tx is values =
  ...
  case runExcept $ txMeasure cfg st tx of
    Left err ->
      ...
      NotProcessed $ TransactionProcessingResult
        Nothing
     ...
     ->
      ...
      ->
        case validateNewTransaction cfg wti tx txsz values st is of
          (Left err, _) ->
            Processed $ \_dur -> TransactionProcessingResult
              Nothing
              (MempoolTxRejected tx err)
              (TraceMempoolRejectedTx
               tx
          ...
          (Right vtx, is') ->
              Processed $ \dur -> TransactionProcessingResult
                (Just (is' dur))
                (MempoolTxAdded vtx)
                (TraceMempoolAddedTx
                  vtx
...
pureRemoveTxs ::
  ...
  -> LedgerTables (LedgerState blk) ValuesMK
  -> TicketNo
  -> [TxTicket (TxMeasureWithDiffTime blk) (Validated (GenTx blk))] -- ^ Txs to keep
