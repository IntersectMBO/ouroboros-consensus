...
implSyncWithLedger ::
  ...
  MempoolEnv m blk ->
  (LedgerState blk EmptyMK, m (Either GetForkerError (ReadOnlyForker m (LedgerState blk) blk))) ->
  m (MempoolSnapshot blk)
implSyncWithLedger mpEnv (ls, meFrk) = encloseTimedWith (TraceMempoolSynced >$< mpEnvTracer mpEnv) $ do
  res <-
    withTMVar istate $
      \is -> do
        eFrk <- meFrk
  ...
  either
    ( \() ->
        do
          st <- atomically (getCurrentLedgerState ldgrInterface $ mpEnvRegistry mpEnv)
