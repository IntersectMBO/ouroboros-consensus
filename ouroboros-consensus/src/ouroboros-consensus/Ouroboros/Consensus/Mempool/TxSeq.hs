...
--
newtype TxSeq sz tx = TxSeq (StrictFingerTree (TxSeqMeasure sz) (TxTicket sz tx))
  deriving stock   (Show)
...
instance Measure sz => FingerTree.Measured (TxSeqMeasure sz) (TxTicket sz tx) where
  measure (TxTicket _ tno tsz) = TxSeqMeasure 1 tno tno tsz
...
instance Measure sz => Semigroup (TxSeqMeasure sz) where
  vl <> vr = TxSeqMeasure
               ...
               (mSizeCumu  vl `Measure.plus` mSizeCumu  vr)

instance Measure sz => Monoid (TxSeqMeasure sz) where
  mempty  = TxSeqMeasure 0 maxBound minBound Measure.zero
  mappend = (<>)
...
-- | \( O(\log(n)) \). Split the sequence of transactions into two parts
-- based on the given size The first part has transactions whose
-- summed size is less than or equal to the given size,
...
splitAfterTxSize (TxSeq txs) n =
    case FingerTree.split (\m -> not $ mSizeCumu m Measure.<= n) txs of
      (l, r) -> (TxSeq l, TxSeq r)
...
splitAfterTxSizeSpec txseq n =
    mapTuple fromList $ go Measure.zero [] (toList txseq)
  where
...
toTuples :: TxSeq sz tx -> [(tx, TicketNo)]
...
toMempoolSize :: Measure sz => TxSeq sz tx -> MempoolSize sz
toMempoolSize (TxSeq ftree) = MempoolSize
    { msNumTxs = fromIntegral mCount
    , msSize   = mSizeCumu
    }
  where
    TxSeqMeasure { mCount, mSizeCumu } = FingerTree.measure ftree
