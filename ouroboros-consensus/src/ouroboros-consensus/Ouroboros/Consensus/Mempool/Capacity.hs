...
data MempoolCapacityBytesOverride
  ...
  | MempoolCapacityBytesOverride !ByteSize
...
-- bytes.
mkCapacityBytesOverride :: ByteSize -> MempoolCapacityBytesOverride
...
computeMempoolCapacity cfg st override =
  ...
  where
    ...
    ByteSize oneBlockBytes = txMeasureByteSize oneBlock
...
instance Monoid MempoolSize where
  mempty  = MempoolSize { msNumTxs = 0, msNumBytes = ByteSize 0 }
  mappend = (<>)
