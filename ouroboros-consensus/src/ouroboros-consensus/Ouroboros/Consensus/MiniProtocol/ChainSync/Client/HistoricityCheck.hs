...
data HistoricityCheck m blk = HistoricityCheck {
    -- | Determine whether the received @MsgRollBackward@ is historical. The
    -- input is the oldest state that was rolled back. (Note that rollbacks of
    -- depth zero are hence never historical.).
...
data HistoricityException =
  -- | We received a @MsgRollBackward@ or a @MsgAwaitReply@ at a time too early
  -- for it to plausibly have been sent by an honest caught-up peer.
  --
  ...
  forall blk. HasHeader blk => HistoricityException {
    ...
    , historicalPoint   :: !(Point blk)
      -- | The time corresponding to the slot of the oldest header that was
      -- rolled back.
    , slotTime          :: !RelativeTime
      -- | When the @MsgRollBackward@ was received.
