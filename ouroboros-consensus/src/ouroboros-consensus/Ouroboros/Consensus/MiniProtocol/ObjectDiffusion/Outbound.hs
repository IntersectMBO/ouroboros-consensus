...
instance Exception ObjectDiffusionProtocolError where
  ...
  displayException (ProtocolErrorRequestedTooManyObjectIds reqNo maxUnacked) =
      "The peer requested " ++ show reqNo ++ " objectIds which would put the "
...
objectDiffusionOutbound tracer maxUnacked MockMempoolReader{..} _version controlMessageSTM =
    ObjectDiffusionClient (pure (client Seq.empty mempoolZeroIndex))
  where
    ...
    client !unackedSeq !lastIndex =
      ...
      where
                ...
                assert (all (\(_, index, _) -> index > lastIndex) objects) $
                  let !unackedSeq'' = unackedSeq' <> Seq.fromList
                      ...
                      !lastIndex'
                        ...
                        | otherwise = index where (_, index, _) = last objects
          ...
          case blocking of
            SingBlocking -> do
              ...
              mbobjects <- timeoutWithControlMessage controlMessageSTM $
                do
                  MockMempoolSnapshot{mempoolObjectIdsAfter} <- mempoolGetSnapshot
                  let objects = mempoolObjectIdsAfter lastIndex
        ...
        recvMsgRequestObjects objectIds = do
          ...
          MockMempoolSnapshot{mempoolLookupObject} <- atomically mempoolGetSnapshot
          ...
          -- The 'mempoolLookupObject' will return nothing if the transaction is no
          ...
          let objects          = mapMaybe mempoolLookupObject objectIndices'
              client'      = client unackedSeq lastIndex
