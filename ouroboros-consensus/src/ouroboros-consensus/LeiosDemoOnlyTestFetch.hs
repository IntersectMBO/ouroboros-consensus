...
instance
  ...
  ShowProxy (LeiosFetch point eb tx)
  where
  showProxy _ =
    concat
      [ "LeiosFetch "
...
deriving instance
  (Eq point, Eq eb, Eq tx) =>
  Eq (Message (LeiosFetch point eb tx) from to)

deriving instance
  (Show point, Show eb, Show tx) =>
...
newtype LeiosFetchRequestHandler point eb tx m
...
leiosFetchClientPeerPipelined tryNext =
 ...
 where
  ...
  send ::
    MutVar (PrimState m) WhetherDraining ->
    Nat n ->
    SomeLeiosFetchJob point eb tx m ->
    X point eb tx m a n
