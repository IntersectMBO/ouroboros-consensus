...
> type instance AuxLedgerEvent BlockD = ()

> instance IsLedger LedgerState BlockD where
>   type instance LedgerErr LedgerState BlockD = Void

>   applyChainTickLedgerResult _events _cfg slot (BlockDStateHandle ldgrSt) =
>     pure LedgerResult { lrEvents = []
>                       , lrResult = TickedBlockDStateHandle
>                           (tickLedgerStateD slot ldgrSt)
>                       }
...
>   applyBlockLedgerResultWithValidation _validation _events _ldgrCfg b
>     (TickedBlockDStateHandle tickedLdgrSt) =
>     pure LedgerResult { lrResult = BlockDStateHandle
>                           (b `applyBlockTo` tickedLdgrSt)
>                       , lrEvents = []
>                       }
...
`BlockD` carries no on-disk component, so the LedgerDB handle layer is
trivial. See the appendix in [the Simple tutorial](./Simple.lhs) for the
rationale behind 'BlockSupportsLedgerHD' and 'LedgerTablesHandle'.

> type instance LedgerTablesHandle m BlockD = ()
