...
main2 =
  getArgs >>= \case
    [dbPath, manifestPath, schedulePath]
      ...
      , ".json" `isSuffixOf` schedulePath ->
          do
            doesFileExist dbPath >>= \case
              ...
              False -> pure ()
            manifest <-
              fmap JSON.eitherDecode (BSL.readFile manifestPath) >>= \case
                Left err -> die err
                Right x -> pure x
    ...
    _ ->
      die
        "usage: leios-schedule-gen my.db myManifest.json outputSchedule.json\n\
        \\n\
        \Generates a fresh SQLite Leios DB and a JSON schedule from the manifest.\n\
        \"
...
instance JSON.FromJSON EbRecipe where
  parseJSON = JSON.withObject "EbRecipe" $ \v ->
    EbRecipe
...
instance JSON.FromJSON EbRecipeElem where
  parseJSON =
   ...
   where
    ...
    share = JSON.withObject "EbRecipeElem" $ \v ->
      EbRecipeShare
      ...
      , let txt = T.pack $ BS8.unpack $ BS16.encode $ Hash.hashToBytes ebHash
         in -- NB reversed!
            MkLeiosScheduleItem (fromIntegral ebSlot + 0.2, (ebSlot, txt, Nothing))
              : MkLeiosScheduleItem
                (fromIntegral ebSlot + 0.1, (ebSlot, txt, Just $ fromIntegral $ BS.length ebBytes))
              : revSchedule
      )
  -- finalize db
...
popRightmostOffset = \case
  0 -> Nothing
  w ->
    let zs = Bits.countTrailingZeros w
     in Just (63 - zs, Bits.clearBit w zs)
...
msgLeiosBlock db lfst0 peerId ebPath = do
  (MkLeiosBlockRequest ebSlot ebHash, lfst1) <-
    case Map.lookup peerId (requestedPerPeer lfst0) of
      Just (LeiosBlockRequest req : reqs) ->
        pure $
          (,) req $
            lfst0
              { requestedPerPeer =
                  if null reqs
                    then Map.delete peerId (requestedPerPeer lfst0)
                    else
  ...
  withDie $ DB.bindInt64 stmt 1 (fromIntegralEbId ebId)
  (ebBytes2, n) <-
    withDiePoly id $ pure $ CBOR.deserialiseFromBytes CBOR.decodeMapLen $ BSL.fromStrict ebBytes
  let go1 txOffset bytes =
        if fromIntegral n == txOffset
          then pure ()
          else do
...
msgLeiosBlockTxs db lfst0 peerId ebTxsPath = do
  (MkLeiosBlockTxsRequest ebSlot ebHash bitmaps0 txHashes, lfst1) <-
    case Map.lookup peerId (requestedPerPeer lfst0) of
      Just (LeiosBlockTxsRequest req : reqs) ->
        pure $
          (,) req $
            lfst0
              { requestedPerPeer =
  ...
  (ebTxsBytes2, ()) <-
    withDiePoly id $ pure $ CBOR.deserialiseFromBytes CBOR.decodeListLenIndef ebTxsBytes
...
instance JSON.FromJSON LeiosFetchState

-- | defaults to @GHC.Generics@
instance JSON.ToJSON LeiosFetchState
  ...
  gen y =
    (,) y $
      Just $
        x
          { ebPoints =
              IntMap.insertWith Map.union (fromIntegral ebSlot) (Map.singleton hashBytes y) (ebPoints x)
  ...
  cached <-
    if not full
      then pure Map.empty
        ...
        loop Map.empty
  (missing, bodies, offsetss) <-
    if not full
      then pure (Set.empty, Map.empty, Map.empty)
      else do
...
-- | defaults to @GHC.Generics@
instance JSON.FromJSON LeiosFetchRequest

-- | defaults to @GHC.Generics@
instance JSON.ToJSON LeiosFetchRequest
...
-- | defaults to @GHC.Generics@
instance JSON.FromJSON LeiosBlockRequest

-- | defaults to @GHC.Generics@
instance JSON.ToJSON LeiosBlockRequest
...
-- | defaults to @GHC.Generics@
instance JSON.FromJSON LeiosBlockTxsRequest

-- | defaults to @GHC.Generics@
instance JSON.ToJSON LeiosBlockTxsRequest

packRequests ::
  LeiosFetchStaticEnv -> LeiosFetchDynamicEnv -> LeiosFetchDecisions -> Map PeerId [LeiosFetchRequest]
        ...
        putStrLn $
          unwords $
            "TX"
              : prettyPeerId peerId
              : show slot
              : prettyHashBytes txHash
              : [prettyEbId ebId ++ "~" ++ show txOffset | (ebId, txOffset) <- Map.toList ebIds]
  acc2 <- (\f -> foldM f acc1 (Map.toList $ packRequests env dynEnv (MkLeiosFetchDecisions decisions))) $ \acc (peerId, reqs) -> do
    forM_ reqs $ \case
      LeiosBlockRequest (MkLeiosBlockRequest ebSlot ebHash) -> do
        putStrLn $
          unwords ["MSG", "MsgLeiosBlockRequest", prettyPeerId peerId, show ebSlot, prettyHashBytes ebHash]
...
msgLeiosBlockOffer acc peerId ebId ebBytesSize = do
  pure
    acc
      ...
      , missingEbBodies =
          ( if Set.member ebId (acquiredEbBodies acc)
              then id
              else
 ...
 where
  ...
  go2 stmt !accBytesSize !accCount !acc ebId txs
    | Just ((txOffset, txBytesSize), txs') <- IntMap.minViewWithKey txs =
        if accBytesSize + txBytesSize > bytesSize
          then stop
          else do
            withDie $ DB.bindInt64 stmt 1 (fromIntegralEbId ebId)
            withDie $ DB.bindInt64 stmt 2 (fromIntegral txOffset)
