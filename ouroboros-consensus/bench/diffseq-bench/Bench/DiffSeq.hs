...
{- | Comparative benchmarks for diff sequence implementations.
  ...
  * Generate a sequence of commands using the warmed up model.
  ...
  phase.
-}
module Bench.DiffSeq (benchmarks) where

import           Bench.Orphans ()
import qualified Bench.SeqUtxoDiff as HD
...
instance (Ord k, Eq v) => IsDiffSeq DS.DiffSeq k v where
  type Diff DS.DiffSeq k v   = Diff.Diff k v
  type Values DS.DiffSeq k v = Map k v
  type Keys DS.DiffSeq k v   = Const (Set k) v
...
genPush = do
  ...
  where
    ...
    valuesToDelete m = do
      ...
      Diff.fromListDeletes . take nrToDelete <$> shuffle (Map.toList m)

    valuesToInsert :: CmdGen k v (Diff DS.DiffSeq k v)
    valuesToInsert = do
      PushConfig{nrToInsert} <- reader pushConfig
      Diff.fromListInserts <$> replicateM nrToInsert arbitrary'
...
genForward = do
    ...
    ks <- keysToForward (Const $ Diff.keysSet $ totalDiff ds)

    let
      vs = Map.restrictKeys bvs $ getConst ks
  ...
  where
    ...
    keysToForward (Const s) = do
      ForwardConfig{nrToForward} <- reader forwardConfig
      Const . Set.fromList . take nrToForward <$> shuffle (toList s)
...
fromDiffSeqCmd cmd = case cmd of
  ...
  Forward vs ks -> Forward (fromValues vs) (fromKeys . getConst $ ks)
...
fromDiff :: Diff.Diff k v -> HD.UtxoDiff k v
fromDiff (Diff.Diff m) = HD.UtxoDiff (fmap fromNEDiffHistory m)

fromNEDiffHistory :: Diff.NEDiffHistory v -> HD.UtxoEntryDiff v
fromNEDiffHistory (Diff.NEDiffHistory neseq) = case neseq of
...
fromKeys :: Set k -> HD.UtxoKeys k v2
