...
import Cardano.Node.Configuration.Socket (SocketConfig (..), mapSocketPath)
...
instance FromJSON CheckpointsFile where
  ...
  parseJSON invalid =
    fail $
      "Parsing of CheckpointsFile failed due to type mismatch. "
        <> "Encountered: "
        <> show invalid
...
instance FromJSON NodeDiffusionMode where
  parseJSON (String str) =
    case str of
      ...
      _ ->
        fail "Parsing NodeDiffusionMode failed: can be either 'InitiatorOnly' or 'InitiatorAndResponder'"
