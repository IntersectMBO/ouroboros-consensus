    ...
    case top of
      RealNodeTopology
        ...
        , ntPeerSnapshotPath
          ...
          object
            [ "localRoots" .= ntLocalRootPeersGroups
    ...
    liftIO $
      CT.traceWith tracer $
        NetworkConfigUpdateInfo $
          createMsg
            "Ledger peers and peer snapshot, although specified in the topology file, are disabled in line with recommended BP operation"
  ...
  when (inGenesisMode && not (useLedgerPeers ntUseLedgerPeers) && not isBlockProducer) $
    liftIO $
      CT.traceWith tracer $
        NetworkConfigUpdateWarning $
          createMsg
            "It is recommended to use ledger peers and peer snapshot file for relay operations in Genesis mode"

  when
    ( inGenesisMode
        && isNothing ntPeerSnapshotPath
        && useLedgerPeers ntUseLedgerPeers
        && not isBlockProducer
    )
    $ liftIO
    $ CT.traceWith tracer
...
isValidTrustedPeerConfiguration (RealNodeTopology (LocalRootPeersGroups lprgs) _ _ ubp _) =
 ...
 where
  anyTrustable =
    any
      ( \LocalRootPeersGroup{localRoots, trustable} ->
          case trustable of
