...
instance AdjustFilePaths PartialNodeConfiguration where
  adjustFilePaths f x =
    x
      { pncProtocolConfig = adjustFilePaths f (pncProtocolConfig x)
  ...
  parseJSON =
    withObject "PartialNodeConfiguration" $ \v -> do
      ...
      pncExperimentalProtocolsEnabled <- fmap Last $ do
        ...
        when (isJust mOldValue) $ do
          when (mOldValue /= mValue) $
            fail
              "TestEnableDevelopmentNetworkProtocols has been renamed to ExperimentalProtocolsEnabled in the configuration file"
      ...
      pncDeadlineTargetOfEstablishedBigLedgerPeers <-
        Last <$> v .:? "TargetNumberOfEstablishedBigLedgerPeers"
      ...
      pncSyncTargetOfEstablishedBigLedgerPeers <-
        Last <$> v .:? "SyncTargetNumberOfEstablishedBigLedgerPeers"
    ...
    parseLedgerDbConfig v = do
      ...
      case mLedgerDB of
        ...
        Just ledgerDB -> flip (withObject "LedgerDB") ledgerDB $ \o -> do
          ldbSnapInterval <-
            (getLast . (Last mTopLevelSnapInterval <>) . Last <$> snapInterval o) .!= DefaultSnapshotInterval
          ldbSnapNum <-
            (getLast . (Last mTopLevelSnapNum <>) . Last <$> snapNum o) .!= DefaultNumOfDiskSnapshots
          qsize <-
            (fmap RequestedQueryBatchSize <$> o .:? "QueryBatchSize") .!= DefaultQueryBatchSize
...
defaultPartialNodeConfiguration =
  PartialNodeConfiguration
    ...
    , -- https://ouroboros-network.cardano.intersectmbo.org/ouroboros-network/Ouroboros-Network-Diffusion-Configuration.html#v:defaultDeadlineTargets
      ...
      pncSyncTargetOfRootPeers =
        Last (Just $ targetNumberOfRootPeers Cardano.defaultSyncTargets)
    , pncSyncTargetOfKnownPeers =
        Last (Just $ targetNumberOfKnownPeers Cardano.defaultSyncTargets)
    , pncSyncTargetOfEstablishedPeers =
        Last (Just $ targetNumberOfEstablishedPeers Cardano.defaultSyncTargets)
    , pncSyncTargetOfActivePeers =
        Last (Just $ targetNumberOfActivePeers Cardano.defaultSyncTargets)
    , pncSyncTargetOfKnownBigLedgerPeers =
        Last (Just $ targetNumberOfKnownBigLedgerPeers Cardano.defaultSyncTargets)
    , pncSyncTargetOfEstablishedBigLedgerPeers =
        Last (Just $ targetNumberOfEstablishedBigLedgerPeers Cardano.defaultSyncTargets)
    , pncSyncTargetOfActiveBigLedgerPeers =
        Last (Just $ targetNumberOfActiveBigLedgerPeers Cardano.defaultSyncTargets)
...
makeNodeConfiguration pnc = do
  ...
  startAsNonProducingNode <-
    lastToEither "Missing StartAsNonProducingNode" $ pncStartAsNonProducingNode pnc
  ...
  ncResponderCoreAffinityPolicy <-
    lastToEither "Missing ResponderCoreAffinityPolicy" $ pncResponderCoreAffinityPolicy pnc
  ...
  unless
    ( PeerSelection.sanePeerSelectionTargets deadlineTargets
        && PeerSelection.sanePeerSelectionTargets syncTargets
    )
    $ Left
    $ "Invalid peer selection targets. Ensure that targets satisfy the "
