...
progressForHuman = round2 . progressForMachine
 where
...
instance MetaTrace ReplayBlockStats where
  ...
  documentFor (Namespace _ ["LedgerReplay"]) =
    Just
...
withReplayedBlock tr = do
 ...
 where
  ...
  mbProduceBlockStats ::
    ChainDB.TraceEvent blk -> ReplayBlockState -> (ReplayBlockState, Maybe ReplayBlockStats)
  mbProduceBlockStats
    ...
    st@(ReplayBlockState mLastSlot) =
      let curSlotNo = realPointSlot curSlot
          ...
          shouldEmit =
            maybe
              True
              (\lastSlot -> progressFor curSlotNo goalSlotNo - progressFor lastSlot goalSlotNo >= 0.01)
              mLastSlot
       in if shouldEmit
