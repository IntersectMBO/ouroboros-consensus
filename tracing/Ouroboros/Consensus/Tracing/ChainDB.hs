...
instance
  ...
  LogFormatting (ChainDB.TraceEvent blk)
  where
  ...
  forMachine dtal (ChainDB.TraceChainSelStarvationEvent (ChainDB.ChainSelStarvation edge)) =
    mconcat
      [ "kind" .= String "ChainSelStarvation"
      , case edge of
...
instance MetaTrace (ChainDB.TraceEvent blk) where
  ...
  documentFor (Namespace _ ["LastShutdownUnclean"]) =
    Just $
      mconcat
  ...
  documentFor (Namespace _ ["ChainSelStarvationEvent"]) =
    Just $
      mconcat
...
instance LogFormatting (PraosReasonForSwitch c) where
  ...
  forMachine _dtal (HigherOCert (Comparing ref cand)) =
    mconcat
      [ "reason" .= String "HigherOCert"
      , "our" .= String (showT ref)
      , "candidate" .= String (showT cand)
      ]
  forMachine _dtal (VRFTiebreak (Comparing ref cand)) =
    mconcat
      [ "reason" .= String "VRFTiebreak"
      , "our" .= String (showT ref)
      , "candidate" .= String (showT cand)
      ]
...
instance
  LogFormatting (ReasonForSwitch (TiebreakerView proto)) =>
  LogFormatting (WeightedSelectViewReasonForSwitch proto)
  where
...
instance
  ...
  LogFormatting
    ( Either
        ...
        (SelectViewReasonForSwitch proto)
    )
  where
  ...
  forMachine _dtal (Right (Longer (Comparing ref cand))) =
    mconcat
      ["reason" .= String "Longer", "our" .= String (showT ref), "candidate" .= String (showT cand)]
...
instance
  ...
  , LogFormatting
      ( Either
          (WithEmptyFragmentReasonForSwitch (WeightedSelectView (BlockProtocol blk)))
  ...
  LogFormatting (ChainDB.TraceAddBlockEvent blk)
  where
  ...
  forHuman (ChainDB.AddedToCurrentChain es _ _ c _) =
    "Chain extended, new tip: "
      <> renderPointAsPhrase (AF.headPoint c)
      <> Text.concat ["\nEvent: " <> showT e | e <- es]
  ...
  forMachine _dtal ChainDB.PoppingFromQueue =
    mconcat
      [ "kind" .= String "PoppingFromQueue"
      ]
  forMachine dtal (ChainDB.PoppedBlockFromQueue pt) =
    mconcat
      [ "kind" .= String "TraceAddBlockEvent.PoppedBlockFromQueue"
      , "block" .= forMachine dtal pt
      ]
  forMachine dtal (ChainDB.StoreButDontChange pt) =
    mconcat
      [ "kind" .= String "StoreButDontChange"
      , "block" .= forMachine dtal pt
      ]
  forMachine dtal (ChainDB.TryAddToCurrentChain pt) =
    mconcat
      [ "kind" .= String "TryAddToCurrentChain"
      , "block" .= forMachine dtal pt
      ]
  ...
  forMachine dtal (ChainDB.ChangingSelection pt) =
    mconcat
      [ "kind" .= String "TraceAddBlockEvent.ChangingSelection"
      , "block" .= forMachine dtal pt
      ]
  ...
  forMachine dtal (ChainDB.AddedBlockToVolatileDB pt (BlockNo bn) _ enclosing) =
    mconcat $
      [ "kind" .= String "AddedBlockToVolatileDB"
      , "block" .= forMachine dtal pt
      , "blockNo" .= showT bn
      ]
  ...
  forMachine _dtal (ChainDB.AddedReprocessLoEBlocksToQueue edgeSz) =
    mconcat
      [ "kind" .= String "AddedReprocessLoEBlocksToQueue"
      , case edgeSz of
          RisingEdge -> "risingEdge" .= True
          FallingEdgeWith sz -> "queueSize" .= toJSON sz
      ]
  ...
  forMachine dtal (ChainDB.ChainSelectionLoEDebug curChain loeFrag) =
    case loeFrag of
      ChainDB.LoEEnabled loeF ->
        mconcat
          [ "kind" .= String "ChainSelectionLoEDebug"
...
instance MetaTrace (ChainDB.TraceAddBlockEvent blk) where
  ...
  severityFor (Namespace _ ["SwitchedToAFork"]) _ =
    Just Notice
  severityFor
    (Namespace out ("AddBlockValidation" : tl))
  ...
  documentFor (Namespace _ ["IgnoreBlockOlderThanImmTip"]) =
    Just
      "A block with a 'BlockNo' not newer than the immutable tip was ignored."
  documentFor (Namespace _ ["IgnoreBlockAlreadyInVolatileDB"]) =
    Just
      "A block that is already in the Volatile DB was ignored."
  documentFor (Namespace _ ["IgnoreInvalidBlock"]) =
    Just
      "A block that is invalid was ignored."
  documentFor (Namespace _ ["AddedBlockToQueue"]) =
    Just $
      mconcat
  ...
  documentFor (Namespace _ ["AddedToCurrentChain"]) =
    Just $
      mconcat
        [ "The new block fits onto the current chain (first"
        , " fragment) and we have successfully used it to extend our (new) current"
  ...
  documentFor (Namespace _out ["SwitchedToAFork"]) =
    Just $
      mconcat
  ...
  allNamespaces =
    ...
    , Namespace [] ["ChainSelectionLoEDebug"]
      ...
      ++ map
        (nsPrependInner "PipeliningEvent")
        (allNamespaces :: [Namespace (ChainDB.TracePipeliningEvent blk)])
      ++ map
        (nsPrependInner "AddBlockValidation")
...
instance
  ...
  LogFormatting (ChainDB.TracePipeliningEvent blk)
  where
...
instance MetaTrace (ChainDB.TracePipeliningEvent blk) where
  ...
  documentFor (Namespace _ ["SetTentativeHeader"]) =
    Just
      "A new tentative header got set"
  documentFor (Namespace _ ["TrapTentativeHeader"]) =
    Just
      "The body of tentative block turned out to be invalid."
  documentFor (Namespace _ ["OutdatedTentativeHeader"]) =
    Just
...
instance MetaTrace (ChainDB.TraceFollowerEvent blk) where
  ...
  documentFor (Namespace _ ["NewFollower"]) =
    Just
      "A new follower was created."
  documentFor (Namespace _ ["FollowerNoLongerInMem"]) =
    Just $
      mconcat
        [ "The follower was in 'FollowerInMem' state and is switched to"
        , " the 'FollowerInImmutableDB' state."
        ]
  documentFor (Namespace _ ["FollowerSwitchToMem"]) =
    Just $
      mconcat
        [ "The follower was in the 'FollowerInImmutableDB' state and is switched to"
        , " the 'FollowerInMem' state."
        ]
  documentFor (Namespace _ ["FollowerNewImmIterator"]) =
    Just $
      mconcat
        [ "The follower is in the 'FollowerInImmutableDB' state but the iterator is"
        , " exhausted while the ImmDB has grown, so we open a new iterator to"
...
instance
  ConvertRawHash blk =>
  LogFormatting (ChainDB.TraceCopyToImmutableDBEvent blk)
  where
  ...
  forMachine dtals (ChainDB.CopiedBlockToImmutableDB pt) =
    mconcat
      [ "kind" .= String "CopiedBlockToImmutableDB"
...
instance MetaTrace (ChainDB.TraceCopyToImmutableDBEvent blk) where
  ...
  documentFor (Namespace _ ["CopiedBlockToImmutableDB"]) =
    Just
      "A block was successfully copied to the ImmDB."
  documentFor (Namespace _ ["NoBlocksToCopyToImmutableDB"]) =
    Just
...
instance MetaTrace (ChainDB.TraceGCEvent blk) where
  ...
  documentFor (Namespace _ ["PerformedGC"]) =
    Just
      "A garbage collection for the given 'SlotNo' was performed."
  documentFor (Namespace _ ["ScheduledGC"]) =
    Just $
      mconcat
...
instance
  ...
  LogFormatting (ChainDB.TraceInitChainSelEvent blk)
  where
...
instance MetaTrace (ChainDB.TraceInitChainSelEvent blk) where
  ...
  documentFor (Namespace _ ["InitialChainSelected"]) =
    Just
      "A garbage collection for the given 'SlotNo' was performed."
  documentFor (Namespace _ ["StartedInitChainSelection"]) =
    Just $
      mconcat
        [ "A garbage collection for the given 'SlotNo' was scheduled to happen"
        , " at the given time."
...
instance
  ...
  LogFormatting (ChainDB.TraceValidationEvent blk)
  where
  ...
  forHuman
    ( ChainDB.UpdateLedgerDbTraceEvent
      ...
      let fromSlot = unSlotNo $ realPointSlot start
       ...
       in "Pushing ledger state for block "
  ...
  forMachine dtal (ChainDB.InvalidBlock err pt) =
    mconcat
      [ "kind" .= String "InvalidBlock"
      , "block" .= forMachine dtal pt
      , "error" .= showT err
      ]
  forMachine dtal (ChainDB.ValidCandidate c) =
    mconcat
      [ "kind" .= String "ValidCandidate"
  ...
  forMachine
    ...
    ( ChainDB.UpdateLedgerDbTraceEvent
      ...
      mconcat
        [ "kind" .= String "UpdateLedgerDbTraceEvent.StartedPushingBlockToTheLedgerDb"
...
instance MetaTrace (ChainDB.TraceValidationEvent blk) where
  ...
  documentFor (Namespace _ ["ValidCandidate"]) =
    Just $
      mconcat
        [ "An event traced during validating performed while adding a block."
        , " A candidate chain was valid."
        ]
  documentFor (Namespace _ ["InvalidBlock"]) =
    Just $
      mconcat
        [ "An event traced during validating performed while adding a block."
...
instance
  ...
  forHuman (ChainDB.OpenedImmutableDB immTip chunk) =
    "Opened imm db with immutable tip at "
      ...
      <> " and chunk "
      <> showT chunk
  forHuman (ChainDB.OpenedVolatileDB mx) =
    "Opened " <> case mx of
...
instance MetaTrace (ChainDB.TraceOpenEvent blk) where
  ...
  documentFor (Namespace _ ["OpenedDB"]) =
    Just
      "The ChainDB was opened."
  documentFor (Namespace _ ["ClosedDB"]) =
    Just
      "The ChainDB was closed."
  documentFor (Namespace _ ["OpenedImmutableDB"]) =
    Just
      "The ImmDB was opened."
  documentFor (Namespace _ ["OpenedVolatileDB"]) =
    Just
      "The VolatileDB was opened."
  documentFor (Namespace _ ["OpenedLgrDB"]) =
    Just
      "The LedgerDB was opened."
  documentFor (Namespace _ ["StartedOpeningDB"]) =
    Just
      "The ChainDB is being opened."
  documentFor (Namespace _ ["StartedOpeningImmutableDB"]) =
    Just
      "The ImmDB is being opened."
  documentFor (Namespace _ ["StartedOpeningVolatileDB"]) =
    Just
      "The VolatileDB is being opened."
  documentFor (Namespace _ ["StartedOpeningLgrDB"]) =
    Just
...
instance
  ...
  LogFormatting (ChainDB.TraceIteratorEvent blk)
  where
...
instance MetaTrace (ChainDB.TraceIteratorEvent blk) where
  ...
  severityFor
    (Namespace out ("UnknownRangeRequested" : tl))
  ...
  privacyFor (Namespace out ("UnknownRangeRequested" : tl)) Nothing =
    privacyFor
      ( Namespace out tl ::
          Namespace (ChainDB.UnknownRange blk)
      )
      Nothing
  privacyFor _ _ = Just Public
  ...
  documentFor (Namespace _ ["StreamFromVolatileDB"]) =
    Just
      "Stream only from the VolatileDB."
  documentFor (Namespace _ ["StreamFromImmutableDB"]) =
    Just
      "Stream only from the ImmDB."
  documentFor (Namespace _ ["StreamFromBoth"]) =
    Just
      "Stream from both the VolatileDB and the ImmDB."
  documentFor (Namespace _ ["BlockMissingFromVolatileDB"]) =
    Just $
      mconcat
  ...
  allNamespaces =
    ...
    , Namespace [] ["SwitchBackToVolatileDB"]
      ...
      ++ map
        (nsPrependInner "UnknownRangeRequested")
...
instance
  ...
  LogFormatting (ChainDB.UnknownRange blk)
  where
  ...
  forMachine _dtal (ChainDB.MissingBlock realPt) =
    mconcat
      [ "kind" .= String "MissingBlock"
      , "point" .= String (renderRealPoint realPt)
      ]
  forMachine _dtal (ChainDB.ForkTooOld streamFrom) =
    mconcat
      [ "kind" .= String "ForkTooOld"
      , "from" .= String (showT streamFrom)
      ]
...
instance MetaTrace (ChainDB.UnknownRange blk) where
  ...
  documentFor (Namespace _ ["MissingBlock"]) =
    Just
      ""
  documentFor (Namespace _ ["ForkTooOld"]) =
    Just
      ""
  documentFor _ = Nothing
...
instance
  ...
  LogFormatting (LedgerDB.TraceEvent blk)
  where
...
instance
  ...
  forMachine dtals (LedgerDB.DeletedSnapshot snap) =
    mconcat
      [ "kind" .= String "DeletedSnapshot"
      , "snapshot" .= forMachine dtals snap
      ]
  forMachine dtals (LedgerDB.InvalidSnapshot snap failure) =
    mconcat
      [ "kind" .= String "InvalidSnapshot"
      , "snapshot" .= forMachine dtals snap
...
instance MetaTrace (LedgerDB.TraceSnapshotEvent blk) where
  ...
  documentFor (Namespace _ ["TookSnapshot"]) =
    Just $
      mconcat
  ...
  documentFor (Namespace _ ["DeletedSnapshot"]) =
    Just
      "A snapshot was deleted from the disk."
  documentFor (Namespace _ ["InvalidSnapshot"]) =
    Just $
      mconcat
  ...
  documentFor (Namespace _ ["SnapshotRequestCompleted"]) =
    Just
...
instance
  ...
  LogFormatting (LedgerDB.TraceReplayEvent blk)
  where
...
instance
  (StandardHash blk, ConvertRawHash blk) =>
  LogFormatting (LedgerDB.TraceReplayStartEvent blk)
  where
  ...
  forMachine dtal (LedgerDB.ReplayFromSnapshot snap tip') =
    mconcat
      [ "kind" .= String "ReplayFromSnapshot"
...
instance MetaTrace (LedgerDB.TraceReplayStartEvent blk) where
  ...
  documentFor (Namespace _ ["ReplayFromGenesis"]) =
    Just $
      mconcat
  ...
  documentFor (Namespace _ ["ReplayFromSnapshot"]) =
    Just $
      mconcat
...
instance MetaTrace (LedgerDB.TraceReplayProgressEvent blk) where
  ...
  documentFor (Namespace _ ["ReplayedBlock"]) =
    Just $
      mconcat
...
instance LogFormatting LedgerDB.TraceForkerEvent where
  ...
  forMachine _dtals (LedgerDB.ForkerRangeReadTables e) =
    mconcat
      [ "kind" .= String "ForkerRangeReadTables"
      , "edge" .= case e of
          RisingEdge -> String "RisingEdge"
          FallingEdgeWith t -> toJSON t
      ]
  ...
  forHuman (LedgerDB.ForkerClose wc) =
    "Closed forker, " <> case wc of
...
unwrapV2Trace ::
  forall a backend. Typeable backend => (V2.Trace LSM.LSM -> a) -> V2.Trace backend -> a
unwrapV2Trace g ev =
...
instance
  (ConvertRawHash blk, StandardHash blk) =>
  LogFormatting (ImmDB.TraceEvent blk)
  where
  ...
  forMachine _dtal (ImmDB.ValidatedLastLocation chunkNo immTip) =
    mconcat
      [ "kind" .= String "ValidatedLastLocation"
  ...
  forMachine _dtal (ImmDB.ChunkFileDoesntFit expectPrevHash actualPrevHash) =
    mconcat
      [ "kind" .= String "ChunkFileDoesntFit"
      , "expectedPrevHash"
          .= String
            ( renderChainHash
                ( Text.decodeLatin1
  ...
  forHuman (ImmDB.ChunkValidationEvent e) = case e of
    ImmDB.StartedValidatingChunk chunkNo outOf ->
      "Validating chunk no. "
        ...
        <> showT outOf
        <> ". Progress: "
        <> showProgressT (chunkNoToInt chunkNo) (chunkNoToInt outOf + 1)
        <> "%"
    ImmDB.ValidatedChunk chunkNo outOf ->
      "Validated chunk no. "
        ...
        <> showT outOf
        <> ". Progress: "
        <> showProgressT (chunkNoToInt chunkNo + 1) (chunkNoToInt outOf + 1)
        <> "%"
  ...
  forHuman (ImmDB.ChunkFileDoesntFit ch1 ch2) =
    "Chunk file doesn't fit. The hash of the block "
      <> showT ch2
      <> " doesn't match the previous hash of the first block in the current epoch: "
      <> showT ch1
      <> "."
  ...
  forHuman (ImmDB.TraceCacheEvent ev') =
    "Cache event: " <> case ev' of
...
instance MetaTrace (ImmDB.TraceEvent blk) where
  ...
  documentFor (Namespace _ ["ValidatedLastLocation"]) =
    Just
  ...
  documentFor (Namespace _ ["ChunkFileDoesntFit"]) =
    Just $
      mconcat
        [ "The hash of the last block in the previous epoch doesn't match the"
        , " previous hash of the first block in the current epoch"
        ]
  documentFor (Namespace _ ["Migrating"]) =
    Just
      "Performing a migration of the on-disk files."
  documentFor (Namespace _ ["DeletingAfter"]) =
    Just
      "Delete after"
  documentFor (Namespace _ ["DBAlreadyClosed"]) =
    Just
      ""
  documentFor (Namespace _ ["DBClosed"]) =
    Just
...
instance ConvertRawHash blk => LogFormatting (ImmDB.TraceChunkValidation blk ImmDB.ChunkNo) where
  forMachine _dtal (ImmDB.RewriteSecondaryIndex chunkNo) =
    mconcat
      [ "kind" .= String "TraceImmutableDBEvent.RewriteSecondaryIndex"
      , "chunkNo" .= String (renderChunkNo chunkNo)
      ]
  forMachine _dtal (ImmDB.RewritePrimaryIndex chunkNo) =
    mconcat
      [ "kind" .= String "TraceImmutableDBEvent.RewritePrimaryIndex"
      , "chunkNo" .= String (renderChunkNo chunkNo)
      ]
  forMachine _dtal (ImmDB.MissingPrimaryIndex chunkNo) =
    mconcat
      [ "kind" .= String "TraceImmutableDBEvent.MissingPrimaryIndex"
      , "chunkNo" .= String (renderChunkNo chunkNo)
      ]
  forMachine _dtal (ImmDB.MissingSecondaryIndex chunkNo) =
    mconcat
      [ "kind" .= String "TraceImmutableDBEvent.MissingSecondaryIndex"
      , "chunkNo" .= String (renderChunkNo chunkNo)
      ]
  forMachine _dtal (ImmDB.InvalidPrimaryIndex chunkNo) =
    mconcat
      [ "kind" .= String "TraceImmutableDBEvent.InvalidPrimaryIndex"
      , "chunkNo" .= String (renderChunkNo chunkNo)
      ]
  forMachine _dtal (ImmDB.InvalidSecondaryIndex chunkNo) =
    mconcat
      [ "kind" .= String "TraceImmutableDBEvent.InvalidSecondaryIndex"
      , "chunkNo" .= String (renderChunkNo chunkNo)
      ]
  forMachine
    _dtal
...
instance MetaTrace (ImmDB.TraceChunkValidation blk chunkNo) where
  ...
  documentFor (Namespace _ ["StartedValidatingChunk"]) =
    Just
      ""
  documentFor (Namespace _ ["ValidatedChunk"]) =
    Just
      ""
  documentFor (Namespace _ ["MissingChunkFile"]) =
    Just
      ""
  documentFor (Namespace _ ["InvalidChunkFile"]) =
    Just
      ""
  documentFor (Namespace _ ["MissingPrimaryIndex"]) =
    Just
      ""
  documentFor (Namespace _ ["MissingSecondaryIndex"]) =
    Just
      ""
  documentFor (Namespace _ ["InvalidPrimaryIndex"]) =
    Just
      ""
  documentFor (Namespace _ ["InvalidSecondaryIndex"]) =
    Just
      ""
  documentFor (Namespace _ ["RewritePrimaryIndex"]) =
    Just
      ""
  documentFor (Namespace _ ["RewriteSecondaryIndex"]) =
    Just
      ""
  documentFor _ = Nothing
...
instance MetaTrace ImmDB.TraceCacheEvent where
  ...
  documentFor (Namespace _ ["CurrentChunkHit"]) =
    Just
      "Current chunk found in the cache."
  documentFor (Namespace _ ["PastChunkHit"]) =
    Just
      "Past chunk found in the cache"
  documentFor (Namespace _ ["PastChunkMiss"]) =
    Just
      "Past chunk was not found in the cache"
  documentFor (Namespace _ ["PastChunkEvict"]) =
    Just $
      mconcat
        [ "The least recently used past chunk was evicted because the cache"
        , " was full."
        ]
  documentFor (Namespace _ ["PastChunkExpired"]) =
    Just
      ""
  documentFor _ = Nothing
...
instance MetaTrace (VolDB.TraceEvent blk) where
  ...
  documentFor (Namespace _ ["BlockAlreadyHere"]) =
    Just
      "A block was found to be already in the DB."
  documentFor (Namespace _ ["Truncate"]) =
    Just
      "Truncates a file up to offset because of the error."
  documentFor (Namespace _ ["InvalidFileNames"]) =
    Just
...
chainInformation ::
  ...
  Int64 ->
  ChainInformation
chainInformation selChangedInfo oldFrag frag blocksUncoupledDelta =
  ChainInformation
    ...
    , tipBlockParentHash =
        renderChainHash (Text.decodeLatin1 . B16.encode . toRawHash (Proxy @blk)) $ AF.headHash oldFrag
...
instance
  ...
  LogFormatting (HeaderEnvelopeError blk)
  where
...
instance
  ...
  where
  -- The two Peras arms carry types with no 'LogFormatting' instance to defer
  -- to: 'PerasError' is a type family, so all we can rely on is its 'Show'.
  ...
  forMachine _dtal (ExtValidationErrorPerasCertInBlock err) =
    mconcat
      [ "kind" .= String "ExtValidationErrorPerasCertInBlock"
      , "error" .= String (showT err)
      ]
...
instance
  ...
  LogFormatting (PBFT.PBftValidationErr c)
  where
...
instance
  Show (PBFT.PBftVerKeyHash c) =>
  LogFormatting (PBFT.PBftCannotForge c)
  where
...
instance ConvertRawHash blk => LogFormatting (ChainDB.TraceAddPerasCertEvent blk) where
  forHuman (ChainDB.AddedPerasCertToQueue roundNo boostedBlock _queueSize) =
    "Added Peras certificate for round "
      <> Text.pack (show roundNo)
      <> " boosting block "
      <> renderPoint boostedBlock
      <> " to queue"
  forHuman (ChainDB.PoppedPerasCertFromQueue roundNo boostedBlock) =
    "Popped Peras certificate for round "
      <> Text.pack (show roundNo)
      <> " boosting block "
      <> renderPoint boostedBlock
      <> " from queue"
  forHuman (ChainDB.IgnorePerasCertTooOld roundNo boostedBlock immutableSlot) =
    "Ignored Peras certificate for round "
      <> Text.pack (show roundNo)
      <> " boosting block "
      <> renderPoint boostedBlock
      <> " (too old, immutable slot: "
      <> renderPoint (AF.anchorToPoint immutableSlot)
      <> ")"
  forHuman (ChainDB.PerasCertBoostsCurrentChain roundNo boostedBlock) =
    "Peras certificate for round "
      <> Text.pack (show roundNo)
      <> " boosts current chain block "
      <> renderPoint boostedBlock
  ...
  forHuman (ChainDB.ChainSelectionForBoostedBlock roundNo boostedBlock) =
    "Chain selection for block "
      <> renderPoint boostedBlock
      <> " boosted by Peras certificate from round "
      <> Text.pack (show roundNo)

  forMachine _dtal (ChainDB.AddedPerasCertToQueue roundNo boostedBlock queueSize) =
    mconcat
      [ "kind" .= String "AddedPerasCertToQueue"
      , "round" .= String (Text.pack $ show roundNo)
