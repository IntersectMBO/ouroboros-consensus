...
module Ouroboros.Consensus.Tracing.Era.HardFork ()
where
...
instance
  ( All (LogFormatting `Compose` WrapLedgerWarning) xs
  , All SingleEraBlock xs
  ) =>
  LogFormatting (HardForkLedgerWarning xs)
  where
...
instance
  ( All (LogFormatting `Compose` WrapLedgerUpdate) xs
  , All SingleEraBlock xs
  ) =>
  LogFormatting (HardForkLedgerUpdate xs)
  where
...
instance All (LogFormatting `Compose` WrapCannotForge) xs => LogFormatting (OneEraCannotForge xs) where
  forMachine dtal =
    hcollapse
      . hcmap
        (Proxy @(LogFormatting `Compose` WrapCannotForge))
        (K . forMachine dtal)
  ...
  forMachine dtal forgeStateInfo =
   ...
   where
    ...
    forgeStateInfo' =
      hcollapse
        . hcmap
          (Proxy @(LogFormatting `Compose` WrapForgeStateInfo))
          (K . forMachine dtal)
...
instance
  All (LogFormatting `Compose` WrapForgeStateUpdateError) xs =>
  LogFormatting (OneEraForgeStateUpdateError xs)
  where
  forMachine dtal forgeStateUpdateError =
   ...
   where
    ...
    forgeStateUpdateError' =
      hcollapse
        . hcmap
          (Proxy @(LogFormatting `Compose` WrapForgeStateUpdateError))
          (K . forMachine dtal)
...
instance All (LogFormatting `Compose` WrapTiebreakerView) xs => LogFormatting (OneEraTiebreakerView xs) where
  forMachine dtal =
    hcollapse
      . hcmap
        (Proxy @(LogFormatting `Compose` WrapTiebreakerView))
        (K . forMachine dtal)
