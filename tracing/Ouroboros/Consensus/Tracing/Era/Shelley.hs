...
module Ouroboros.Consensus.Tracing.Era.Shelley () where
...
jsonNonEmptyMap :: (ToJSONKey k, ToJSON v) => NonEmptyMap.NonEmptyMap k v -> Value
jsonNonEmptyMap = toJSON . NonEmptyMap.toMap
...
instance
  ...
  LogFormatting (Conway.ConwayCertPredFailure era)
  where
  forMachine dtal =
    mconcat . \case
...
instance
  ...
  LogFormatting (Header (ShelleyBlock protocol era))
  where
  forMachine _dtal b =
    mconcat
...
instance
  Ledger.Crypto era =>
  LogFormatting (TPraosCannotForge era)
  where
...
instance
  ...
  LogFormatting (BlockTransitionError era)
  where
  forMachine dtal (BlockTransitionError fs) =
    mconcat
      [ "kind" .= String "BlockTransitionError"
...
instance
  ...
  LogFormatting (ChainTransitionError crypto)
  where
...
instance
  ...
  LogFormatting (ShelleyLedgerPredFailure era)
  where
  forMachine dtal = \case
    ...
    ShelleyIncompleteWithdrawals payload ->
      mconcat
        [ "kind" .= String "ShelleyIncompleteWithdrawals"
...
instance
  ...
  forMachine _ (MissingRequiredDatums required received) =
    mconcat
      ...
      , "required"
          .= map
            (Crypto.hashToTextAsHex . Hashes.extractHash)
  ...
  forMachine _ (NotAllowedSupplementalDatums disallowed acceptable) =
    mconcat
      [ "kind" .= String "NotAllowedSupplementalDatums"
...
instance
  ...
  forMachine _dtal (MissingVKeyWitnessesUTXOW wits') =
    mconcat
      ...
      , "missingWitnesses" .= jsonNonEmptySet wits'
      ]
  ...
  forMachine _dtal (ScriptWitnessNotValidatingUTXOW failedScripts) =
    mconcat
      ...
      , "failedScripts" .= jsonNonEmptySet failedScripts
      ]
  ...
  forMachine
    ...
    ( ConflictingMetadataHash
        Mismatch
          { mismatchSupplied = txBodyMetadataHash
  ...
  forMachine _dtal (ExtraneousScriptWitnessesUTXOW scriptHashes) =
    mconcat
      [ "kind" .= String "ExtraneousScriptWitnessesUTXOW"
...
instance
  ...
  LogFormatting (ShelleyUtxoPredFailure era)
  where
  forMachine _dtal (BadInputsUTxO badInputs) =
    mconcat
      [ "kind" .= String "BadInputsUTxO"
      , "badInputs" .= jsonNonEmptySet badInputs
...
instance
  ...
  LogFormatting (ShelleyDelegsPredFailure era)
  where
...
instance
  ...
  LogFormatting (ShelleyDelplPredFailure era)
  where
...
instance LogFormatting (ShelleyDelegPredFailure era) where
  ...
  forMachine _dtal (DuplicateGenesisVRFDELEG vrfKeyHash) =
    mconcat
      ...
      , "keyHash" .= String (Crypto.hashToTextAsHex (Hashes.unVRFVerKeyHash vrfKeyHash))
      ]
...
instance LogFormatting (ShelleyPoolPredFailure era) where
  ...
  forMachine
    ...
    ( StakePoolRetirementWrongEpochPOOL
        ...
        Mismatch
          { mismatchSupplied = intendedRetireEpoch
  ...
  forMachine _dtal (VRFKeyHashAlreadyRegistered poolId vrfKeyHash) =
    mconcat
      [ "kind" .= String "VRFKeyHashAlreadyRegistered"
      , "poolId" .= String (textShow poolId)
...
instance
  Ledger.Crypto crypto =>
  LogFormatting (PrtclPredicateFailure crypto)
  where
...
instance
  Ledger.Crypto crypto =>
  LogFormatting (OverlayPredicateFailure crypto)
  where
...
instance
  ...
  LogFormatting (AlonzoUtxosPredFailure ledgerera)
  where
...
instance
  ...
  LogFormatting (AlonzoBbodyPredFailure era)
  where
  forMachine _ err =
    mconcat
...
instance
  ...
  LogFormatting (BabbageUtxoPredFailure era)
  where
  forMachine v err =
    case err of
      ...
      Babbage.IncorrectTotalCollateralField provided declared ->
        mconcat
          [ "kind" .= String "IncorrectTotalCollateralField"
      ...
      Babbage.BabbageOutputTooSmallUTxO outputs ->
        mconcat
          [ "kind" .= String "OutputTooSmall"
          , "outputs" .= outputs
          ]
      Babbage.BabbageNonDisjointRefInputs nonDisjointInputs ->
        mconcat
          [ "kind" .= String "BabbageNonDisjointRefInputs"
  ...
  forMachine v err =
    case err of
      ...
      Babbage.MalformedScriptWitnesses s ->
        mconcat
          [ "kind" .= String "MalformedScriptWitnesses"
          , "scripts" .= jsonNonEmptySet s
          ]
      Babbage.MalformedReferenceScripts s ->
        mconcat
          [ "kind" .= String "MalformedReferenceScripts"
          , "scripts" .= jsonNonEmptySet s
          ]
...
instance
  ...
  LogFormatting (Conway.ConwayBbodyPredFailure era)
  where
  forMachine _ err =
    mconcat
      [ "kind" .= String "ConwayBbodyPredFail"
      , "error" .= String (textShow err)
      ]
...
instance
  ...
  LogFormatting (Conway.ConwayLedgerPredFailure era)
  where
  ...
  forMachine _ (Conway.ConwayTxRefScriptsSizeTooBig Mismatch{mismatchSupplied, mismatchExpected}) =
    mconcat
      [ "kind" .= String "ConwayTxRefScriptsSizeTooBig"
  ...
  forMachine _ err' =
    case err' of
      ...
      Praos.VRFLeaderValueTooBig leaderValue sigma f ->
        mconcat
          [ "kind" .= String "VRFLeaderValueTooBig"
...
instance LogFormatting (Praos.PraosCannotForge crypto) where
  forMachine _ (Praos.PraosCannotForgeKeyNotUsableYet currentKesPeriod startingKesPeriod) =
    mconcat
      [ "kind" .= String "PraosCannotForgeKeyNotUsableYet"
...
instance
  ToJSON (Alonzo.CollectError ledgerera) =>
  LogFormatting (Conway.ConwayUtxosPredFailure ledgerera)
  where
...
instance
  ...
  LogFormatting (Conway.ConwayUtxoPredFailure ledgerera)
  where
  forMachine dtal = \case
    ...
    Conway.IncorrectTotalCollateralField provided declared ->
      mconcat
        [ "kind" .= String "UnequalCollateralReturn"
        , "collateralProvided" .= provided
        , "collateralDeclared" .= declared
        ]
    Conway.BabbageOutputTooSmallUTxO outputs ->
      mconcat
        [ "kind" .= String "BabbageOutputTooSmall"
        , "outputs" .= outputs
        ]
...
instance
  ...
  LogFormatting (Conway.ConwayUtxowPredFailure ledgerera)
  where
  forMachine dtal = \case
    ...
    Conway.MissingVKeyWitnessesUTXOW ws ->
      mconcat
        [ "kind" .= String "MissingVKeyWitnessesUTXOW"
        , "missingWitnesses" .= jsonNonEmptySet ws
        ]
    Conway.MissingScriptWitnessesUTXOW scripts ->
      mconcat
        [ "kind" .= String "MissingScriptWitnessesUTXOW"
        , "missingScripts" .= jsonNonEmptySet scripts
        ]
    ...
    Conway.ExtraRedeemers rs ->
      mconcat
        [ "kind" .= String "ExtraRedeemers"
        , "rdmrs" .= map renderScriptIndex (NonEmpty.toList rs)
        ]
    Conway.MalformedScriptWitnesses scripts ->
      mconcat
        [ "kind" .= String "MalformedScriptWitnesses"
        , "scripts" .= jsonNonEmptySet scripts
        ]
...
instance LogFormatting (Praos.PraosTiebreakerView crypto) where
  forMachine
    ...
    Praos.PraosTiebreakerView
      ...
      mconcat
        [ "kind" .= String "PraosTiebreakerView"
        , "slotNo" .= ptvSlotNo
