...
genEraParams = do
  ...
  eraPerasRoundLength <-
    frequency
      ...
      , (3, HF.PerasEnabled <$> genPerasRoundLength eraEpochSize)
      ]
  return HF.EraParams{..}
 where
  ...
  -- We restrict Peras round length to divide the epoch size evenly.
  genPerasRoundLength :: EpochSize -> Gen PerasRoundLength
  genPerasRoundLength epochSize =
    PerasRoundLength
      <$> choose (1, 10) `suchThat` \x ->
        unEpochSize epochSize `mod` x == 0
