...
case "$(uname -s)" in
    MINGW*)     path="$(pwd -W | sed 's_/_\\\\_g')\\\\(ouroboros-consensus|sop-extras|strict-sop-core|cardano-config)";;
    *)          path="$(pwd)/(ouroboros-consensus|sop-extras|strict-sop-core|cardano-config)";;
esac
