        ...
        let loop (slot, ledger, mempFp) = do
              (snap1, snap2) <- atomically $ do
                snap1 <- getSnapshotFor mempool $
                  ...
                  -- this slot.
                  ForgeInKnownSlot slot $ applyChainTickWithSTSOpts (fastSTSOpts (Proxy @(LedgerState blk))) lcfg slot ledger
                snap2 <- getSnapshotFor mempool $
                  -- Other nodes might include these crucial txs when leading
                  -- in the next slot.
                  ForgeInKnownSlot (succ slot) $ applyChainTickWithSTSOpts (fastSTSOpts (Proxy @(LedgerState blk))) lcfg (succ slot) ledger
          ...
          customForgeBlock origBlockForging cfg' currentBno currentSlot tickedLdgSt txs prf = do
            ...
            case mbForgeEbbEnv <* guard needEBB of
              ...
              Just forgeEbbEnv -> do
                  ...
                  let apply = applyLedgerBlockWithSTSOpts (fastSTSOpts (Proxy @(LedgerState blk))) (configLedger pInfoConfig)
                  tickedLdgSt' <- case Exc.runExcept $ apply ebb tickedLdgSt of
                    Left e   -> Exn.throw $ JitEbbError @blk e
                    Right st -> pure $ applyChainTickWithSTSOpts
                                        (fastSTSOpts (Proxy @(LedgerState blk)))
