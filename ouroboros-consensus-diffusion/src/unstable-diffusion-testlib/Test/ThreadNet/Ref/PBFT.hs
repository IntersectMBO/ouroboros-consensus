...
import Test.Util.InvertedMap qualified as InvertedMap
...
extendForger params st@State{forgers} i =
  st
...
extendOutcome params State{forgers, nomCount, nextSlot, outs} out =
  State
    { forgers
    , nomCount = nomCount'
    , outs = outs'
...
emptyShortState (NodeJoinPlan m) =
  ShortState
