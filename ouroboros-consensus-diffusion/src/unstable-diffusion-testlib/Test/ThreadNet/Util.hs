...
prop_common_prefix l x y = go x y .&&. go y x
 where
  ...
  findPrefix c' d
    ...
    | otherwise =
        let (l', c'') = findPrefix (Chain.dropLastBlocks 1 c') d
         in (l' + 1, c'')

  showChain :: Chain b -> String
  showChain c =
    condense (Chain.headTip c)
      <> "\n(length "
      <> show (Chain.length c)
      <> ")"
...
emptyLeaderSchedule (NumSlots t) =
  LeaderSchedule $
    Map.fromList $
      [ (SlotNo i, [])
      | i <- [0 .. t - 1]
...
roundRobinLeaderSchedule (NumCoreNodes n) (NumSlots t) =
  LeaderSchedule $
    Map.fromList $
