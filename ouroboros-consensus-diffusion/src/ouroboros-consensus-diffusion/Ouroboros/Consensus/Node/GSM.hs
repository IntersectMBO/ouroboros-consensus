...
realGsmEntryPoints gsmView = GsmEntryPoints {
  ...
  where
    ...
    blockWhileCaughtUp :: Maybe StdGen -> m (Maybe StdGen)
    blockWhileCaughtUp g = do
        ...
        atomically getCurrentSelection >>= blockWhileCaughtUpHelper bonus

        pure g'

    blockWhileCaughtUpHelper :: SI.DiffTime -> selection -> m ()
