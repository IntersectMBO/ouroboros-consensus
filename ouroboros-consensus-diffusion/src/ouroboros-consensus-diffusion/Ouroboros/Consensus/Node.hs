...
--
-- TODO: remove extraArgs (Cardano.ExtraArgs)
data StdRunNodeArgs m blk (p2p :: Diffusion.P2P) extraArgs extraState extraDebugState extraActions extraAPI extraPeers extraFlags extraChurnArgs extraCounters exception = StdRunNodeArgs
  ...
  , srnDiffusionArgumentsExtra      :: Diffusion.P2PDecision p2p (Tracer IO TracePublicRootPeers) ()
                                    ...
                                    -> Diffusion.ArgumentsExtra p2p extraArgs extraState extraDebugState extraFlags extraPeers extraAPI extraChurnArgs extraCounters exception RemoteAddress Resolver IOException IO
  ...
  , srnSigUSR1SignalHandler         :: ( forall (mode :: Mode) x y.
                                          P2P.TracersExtra RemoteAddress NodeToNodeVersion NodeToNodeVersionData
...
stdRunDataDiffusion
  ...
  => ( forall (mode :: Mode) x y.
       P2P.NodeToNodeConnectionManager
         mode
         Socket
         RemoteAddress
         NodeToNodeVersionData
                ...
                case srnDiffusionTracersExtra of
                   Diffusion.P2PTracers extraTracers -> do
                     let srnDiffusionArgumentsExtra' =
                           srnDiffusionArgumentsExtra (Diffusion.P2PDecision (P2P.dtTracePublicRootPeersTracer extraTracers))
                                                      (Diffusion.P2PDecision (getFetchMode kernel))
                                                      (Diffusion.P2PDecision (lpExtraAPI (Common.daLedgerPeersCtx apps)))
                     case srnDiffusionArgumentsExtra' of
                       Diffusion.P2PArguments extraArgs ->
                         stdRunDataDiffusion
                             (srnSigUSR1SignalHandler
                                extraTracers
                                (P2P.daReadUseLedgerPeers extraArgs)
                                rnPeerSharing
                                rnGetUseBootstrapPeers
