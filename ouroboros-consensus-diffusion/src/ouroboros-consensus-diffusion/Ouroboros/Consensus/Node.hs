...
data LowLevelRunNodeArgs m addrNTN addrNTC blk extraAPI =
   LowLevelRunNodeArgs {
...
data StdRunNodeArgs m blk extraArgs extraState extraDebugState extraActions extraAPI extraPeers extraFlags extraChurnArgs extraCounters exception = StdRunNodeArgs
  ...
  , srnDatabasePath                 :: NodeDatabasePaths
    -- ^ Location of the DBs
  , srnDiffusionMode                :: DiffusionMode
    -- ^ This is also available in Diffusion.Arguments, however we can't have
    -- access to it unless we pass the required arguments to
    -- 'srnDiffusionArguments' and those might not be available outside a
    -- closure with access to Applications and NodeKernel. This value doesn't
    -- depend on either so we can require it earlier

  , srnPublicPeerSelectionVar       :: StrictSTM.StrictTVar m (PublicPeerSelectionState RemoteAddress)
    -- ^ This is also available in Diffusion.Arguments, however we can't have
    -- access to it unless we pass the required arguments to
    -- 'srnDiffusionArguments' and those might not be available outside a
    -- closure with access to Applications and NodeKernel. This value doesn't
  ...
  , srnDiffusionArguments           :: Tracer IO TracePublicRootPeers
                                    -> STM IO FetchMode
                                    -> extraAPI
                                    -> Diffusion.Arguments
                                         ...
                                         exception Resolver IOException
                                         IO Socket RemoteAddress LocalSocket
                                         LocalAddress
  , srnDiffusionTracers             :: Diffusion.Tracers
                                         RemoteAddress  NodeToNodeVersion   NodeToNodeVersionData
                                         LocalAddress   NodeToClientVersion NodeToClientVersionData
                                         IOException extraState extraDebugState
                                         extraFlags extraPeers extraCounters IO
  , srnSigUSR1SignalHandler         :: ( forall (mode :: Mode) x y.
                                       ...
                                       -> Diffusion.NodeToNodeConnectionManager mode Socket
                                       ...
                                       -> StrictSTM.StrictTVar IO
                                            (PeerSelectionState extraState extraFlags extraPeers
                                                                RemoteAddress
                                                                (Diffusion.NodeToNodePeerConnectionHandle
                                                                    mode RemoteAddress
...
run :: forall blk extraState extraActions extraPeers extraFlags extraChurnArgs extraCounters exception.
    ( RunNode blk
     , Monoid extraPeers
     , Eq extraCounters
     , Eq extraFlags
...
runWith :: forall m addrNTN addrNTC blk.
  ...
  -> LowLevelRunNodeArgs m addrNTN addrNTC blk (Cardano.LedgerPeersConsensusInterface m)
  -> m Void
      ...
      handleJust
        ...
        continueWithCleanChainDB chainDB $ do
          ...
          let ntnApps = mkNodeToNodeApps   nodeKernelArgs nodeKernel peerMetrics encAddrNtN decAddrNtN
              ...
              apps   =
                mkDiffusionApplications (miniProtocolParameters nodeKernelArgs)
                                        ntnApps
                                        ntcApps
                                        nodeKernel
                                        peerMetrics

          llrnRunDataDiffusion nodeKernel apps
  where
    ProtocolInfo
      { pInfoConfig       = cfg
    ...
    mkDiffusionApplications
      ...
      peerMetrics =
        Diffusion.Applications {
          ...
          Diffusion.daPeerMetrics         = peerMetrics,
          Diffusion.daPeerSharingRegistry = getPeerSharingRegistry kernel,
          Diffusion.daBlockFetchMode      = getFetchMode kernel
        }
...
stdRunDataDiffusion
  ...
  => ( forall (mode :: Mode) x y.
       Diffusion.NodeToNodeConnectionManager
         mode
         Socket
         RemoteAddress
         NodeToNodeVersionData
...
stdLowLevelRunNodeArgsIO
  :: forall blk extraState extraActions extraPeers extraFlags extraChurnArgs extraCounters exception .
  ( RunNode blk
  , Monoid extraPeers
  , Eq extraCounters
  , Eq extraFlags
...
stdLowLevelRunNodeArgsIO RunNodeArgs{ rnProtocolInfo
    ...
    pure LowLevelRunNodeArgs
      ...
      , llrnRunDataDiffusion =
          \kernel apps -> do
            let srnDiffusionArguments' =
                  srnDiffusionArguments (Diffusion.dtTracePublicRootPeersTracer srnDiffusionTracers)
                                        (getFetchMode kernel)
                                        (lpExtraAPI (Diffusion.daLedgerPeersCtx apps))
            stdRunDataDiffusion
              (srnSigUSR1SignalHandler
                srnDiffusionTracers
                (Diffusion.daReadUseLedgerPeers srnDiffusionArguments')
                rnPeerSharing
                rnGetUseBootstrapPeers
