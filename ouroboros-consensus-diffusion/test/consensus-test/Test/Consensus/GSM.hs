...
import qualified Ouroboros.Consensus.Util.STM as W
...
prop_sequential1 j0 cmds = runSimQC $ do
    ...
    let watcher = W.Watcher {
          ...
          ,
            W.wInitial = Nothing
          ,
            W.wNotify = push varEvents . EvGsmWrite
          ,
            W.wReader = (,) <$> readTVar varChanges <*> readTVar varJudgment
          }
    ...
    let gsm = GSM.realGsmEntryPoints GSM.GsmView {
          ...
          ,
            GSM.writeLedgerStateJudgement = \x -> atomically $ do
                -- additional bookkeeping to ensure the watcher reports even
                -- idempotent reads
                y <- readTVar varChanges
                writeTVar varChanges $! (y + 1)
...
data Ev =
  ...
  |
    EvGsmWrite (Int, LedgerStateJudgement)
    -- ^ the GSM code being tested wrote to the TVar for the @n@th time
  deriving (Show)
...
data Vars m = Vars
    ...
    (StrictTVar m Int, StrictTVar m LedgerStateJudgement)
