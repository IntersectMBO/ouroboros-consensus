...
{-------------------------------------------------------------------------------
  Handle layer (trivial — BlockA has no on-disk tables)
-------------------------------------------------------------------------------}

type instance LedgerTablesHandle m BlockA = ()

instance BlockSupportsLedgerHD m BlockA where
  newtype StateHandle m BlockA = BlockAStateHandle (LedgerState BlockA)
    deriving NoThunks via OnlyCheckWhnfNamed "StateHandleA" (StateHandle m BlockA)
  newtype TickedStateHandle m BlockA
    = TickedBlockAStateHandle (Ticked LedgerState BlockA)
    deriving NoThunks via OnlyCheckWhnfNamed "TickedStateHandleA" (TickedStateHandle m BlockA)

  newStateHandle st () = BlockAStateHandle st

  state (BlockAStateHandle s) = s
  tickedState (TickedBlockAStateHandle s) = s
...
instance IsLedger LedgerState BlockA where
  ...
  applyChainTickLedgerResult _ _ _ (BlockAStateHandle st) =
    pure $ pureLedgerResult $ TickedBlockAStateHandle $ TickedLedgerStateA st

instance ApplyBlock LedgerState BlockA where
  applyBlockLedgerResultWithValidation _ _ _cfg blk (TickedBlockAStateHandle (TickedLedgerStateA st0)) =
    let st' = Prelude.foldl applyOne st0 (blkA_body blk)
     in pure $ pureLedgerResult $ BlockAStateHandle $ setTip st'
   where
    setTip :: LedgerState BlockA -> LedgerState BlockA
    setTip st = st{lgrA_tip = blockPoint blk}

    applyOne :: LedgerState BlockA -> GenTx BlockA -> LedgerState BlockA
    applyOne st (TxA _ InitiateAtoB) = st{lgrA_transition = Just (blockSlot blk)}
...
data instance TxLocalData BlockA = BlockATxLocalData
  deriving stock Generic
  deriving anyclass NoThunks

-- | BlockA has no on-disk tables; the acc is the ticked ledger state.
newtype instance MempoolAcc BlockA = BlockAMempoolAcc
  {unBlockAMempoolAcc :: Ticked LedgerState BlockA}
  deriving stock Generic
  deriving anyclass NoThunks

instance LedgerSupportsMempool BlockA where
  emptyAcc = BlockAMempoolAcc
  accTickedState = unBlockAMempoolAcc

  prepareTx _cfg _slot _tsh _acc _tx = pure BlockATxLocalData

  applyTx _ _wti sno (BlockAMempoolAcc (TickedLedgerStateA st)) tx@(TxA _ payload) _tld =
    case payload of
      InitiateAtoB ->
        pure
          ( ValidatedGenTxA tx
          , BlockAMempoolAcc $ TickedLedgerStateA $ st{lgrA_transition = Just sno}
          )

  reapplyTx _cfg sno (BlockAMempoolAcc (TickedLedgerStateA st)) vtx _tld =
    case txA_payload (forgetValidatedGenTxA vtx) of
      InitiateAtoB ->
        pure $ BlockAMempoolAcc $ TickedLedgerStateA $ st{lgrA_transition = Just sno}
