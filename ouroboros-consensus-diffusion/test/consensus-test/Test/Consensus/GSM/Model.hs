...
import Data.TreeDiff qualified as TD
...
import Test.StateMachine.Types.Rank2 qualified as QSM
...
precondition ctx model = pre $ \case
  cmd@ExtendSelection{} ->
    let model' = transition ctx model cmd Unit
     in "syncing node got ahead"
          `atom` selectionIsBehind model
          QSM..&& "early selection"
          `atom` selectionIsNotEarly model'
          QSM..&& boringDur model' 0
  Disconnect peer ->
  ...
  StartIdling peer ->
    "idle after disconnect"
      `atom` (peer `Map.member` cands)
      QSM..&& "double idle"
      `atom` (peer `Set.notMember` idlers)
  TimePasses dur ->
    "non-positive duration"
      `atom` (0 < dur)
      QSM..&& boringDur model dur
 where
  Model
    { mCandidates = cands
    , mIdlers = idlers
  ...
  boringState = case st of
    ...
    ModelCaughtUp timestamp ->
      let gap = clk' `SI.diffTime` expiry timestamp
       ...
       in "boringDur state" `atom` (gap < 0 || 0 /= n)
