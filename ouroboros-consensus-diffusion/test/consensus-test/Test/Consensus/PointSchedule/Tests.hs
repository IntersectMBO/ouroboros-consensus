...
import Test.Util.QuickCheck qualified as QC
...
prop_zipMany xss =
  let xs :: [Int]
      ...
      ys :: [[(Int, Int)]]
      ys = zipMany xs xss
   in length xss QC.=== length ys
        QC..&&. map (map snd) ys QC.=== xss
        QC..&&. concatMap (map fst) ys QC.=== xs
...
prop_singleJumpTipPoints seed (SingleJumpTipPointsInput m n) =
  runSTGen_ seed $ \g -> do
    ...
    pure $
      isSorted QC.le xs
        QC..&&. ( QC.counterexample ("length xs = " ++ show (length xs)) $
                    length xs `QC.le` n - m + 1
                )
        QC..&&. ( QC.counterexample ("head xs = " ++ show (headCallStack xs)) $
                    headCallStack xs `QC.le` n
                      QC..&&. m `QC.le` headCallStack xs
                )
  ...
  runSTGen_ g $ \g' -> do
    ...
    pure $
      ( QC.counterexample ("length xs = " ++ show (length xs)) $
        ...
        QC..&&. ( QC.counterexample ("header points are sorted in each branch") $
                    foldr
                      (QC..&&.)
                      (QC.property True)
        ...
        QC..&&. ( QC.counterexample ("times are sorted accross branches") $
        ...
        QC..&&. ( QC.counterexample ("trunk header points are sorted accross branches") $
                    QC.counterexample ("branches = " ++ show hpss) $
                      isSorted QC.lt $
                        concat
                          [map snd trunk | HeaderPointSchedule trunk _ <- hpss]
                )
        QC..&&. ( QC.counterexample "branch header points follow tip points" $
                    QC.counterexample ("branches = " ++ show hpss) $
...
data PeerScheduleFromTipPointsInput
    ...
    pure $
      ( QC.counterexample ("hps = " ++ show (map (second showPoint) hps)) $
          QC.counterexample ("tps' = " ++ show (map (second showPoint) tps')) $
            headerPointsFollowTipPoints
              isAncestorBlock'
              (map (second schedulePointToBlock) hps)
              (map (second schedulePointToBlock) tps')
      )
        QC..&&. ( QC.counterexample ("schedule = " ++ show (map (second showPoint) ss)) $
  ...
  go ancestors (p : ss) =
    let b = schedulePointToBlock p
     in foldr
          ...
          QC..&&. go (b : ancestors) ss

  isNotAncestorOf' :: WithOrigin TestBlock -> WithOrigin TestBlock -> QC.Property
  isNotAncestorOf' b0 b1 =
    QC.counterexample
      ("return to ancestor: " ++ terseWithOrigin terseBlock b0 ++ " -> " ++ terseWithOrigin terseBlock b1)
      $ QC.property
      $ isNothing
      $ isAncestorBlock' b0 b1
...
headerPointsFollowTipPoints ::
  Show a => (a -> a -> Maybe Ordering) -> [(Time, a)] -> [(Time, a)] -> QC.Property
...
genAdversarialFragment ::
  AF.AnchoredFragment TestBlock -> Int -> Int -> [SlotNo] -> AF.AnchoredFragment TestBlock
...
mkFragment pre active forkNo = AF.fromNewestFirst anchor $ List.foldl' issue [] active
 where
  ...
  issue [] s
    | Origin <- pre = [(firstBlock (fromIntegral forkNo)){tbSlot = s}]
