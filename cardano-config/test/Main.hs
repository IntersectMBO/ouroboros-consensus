...
main = do
  results <-
    sequence
      [ decodeCase "examples/storage.json" (eitherDecodeFileStrict' "examples/storage.json" :: IO (Either String (StorageConfiguration Maybe)))
      , decodeCase "examples/consensus.json" (eitherDecodeFileStrict' "examples/consensus.json" :: IO (Either String (ConsensusConfiguration Maybe)))
      , decodeCase "examples/protocol.json" (eitherDecodeFileStrict' "examples/protocol.json" :: IO (Either String (ProtocolConfiguration Maybe)))
