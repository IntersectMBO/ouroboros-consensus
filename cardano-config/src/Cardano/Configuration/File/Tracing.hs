...
-- | The tracing configuration is given under a single @HermodTracing@ key, whose
-- value is either an inline object or a path (a string) to a separate file
-- holding that object. It is captured opaquely; see the module documentation.
newtype TracingConfiguration = TracingConfiguration
  { hermodTracing :: Maybe (Either Text Value)
  -- ^ A path to a file holding the tracing configuration ('Left'), or an inline
  -- object ('Right').
  }
    ...
    object "TracingConfiguration" $
      TracingConfiguration
        <$> optionalFieldWith
          ...
          ( "Tracing configuration as an inline object, or a path to a separate file holding it. "
