...
deriving via (Autodocodec (ConsensusConfiguration Maybe)) instance FromJSON (ConsensusConfiguration Maybe)

deriving via (Autodocodec (ConsensusConfiguration Maybe)) instance ToJSON (ConsensusConfiguration Maybe)
  ...
  codec =
    object "GenesisConfigFlags" $
      GenesisConfigFlags
        ...
        <*> optionalFieldWithDefault "EnableLoEAndGDD" True "Enable the Limit on Eagerness and the Genesis Density Disconnection" .= gcfEnableLoEAndGDD
        <*> optionalFieldWithDefault "EnableLoP" True "Enable the Limit on Patience" .= gcfEnableLoP
        <*> optionalFieldWith "BlockFetchGracePeriod" diffTimeCodec "Grace period, in seconds, for BlockFetch" .= gcfBlockFetchGracePeriod
        ...
        <*> optionalFieldWith "GDDRateLimit" diffTimeCodec "Rate limit, in seconds, for the GDD" .= gcfGDDRateLimit
