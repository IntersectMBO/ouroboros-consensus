...
import Cardano.Chain.Block qualified as Block
...
import Ouroboros.Consensus.Byron.Crypto.DSIGN qualified as Crypto
...
import Ouroboros.Consensus.Protocol.PBFT.Crypto qualified as Crypto
...
import Test.ThreadNet.Ref.PBFT qualified as Ref
...
import Test.Util.Stream qualified as Stream
...
genTestSetup k numCoreNodes numSlots setupSlotLength = do
  ...
  nodeRestarts <-
    genNodeRestarts nodeJoinPlan numSlots
            ...
            let ncn = NumCoreNodes 3
             in prop_simple_real_pbft_convergence
                  TestSetup
                    ...
                    , setupNodeJoinPlan =
                        NodeJoinPlan $
                          Map.fromList [(CoreNodeId 0, SlotNo 0), (CoreNodeId 1, SlotNo 20), (CoreNodeId 2, SlotNo 22)]
    ...
    , testProperty "repeatedly add the the dlg cert tx" $
        ...
        prop_simple_real_pbft_convergence
          TestSetup
            ...
            , setupNodeJoinPlan =
                NodeJoinPlan $
                  Map.fromList [(CoreNodeId 0, SlotNo 0), (CoreNodeId 1, SlotNo 0), (CoreNodeId 2, SlotNo 83)]
            , setupNodeRestarts =
                NodeRestarts $ Map.fromList [(SlotNo 83, Map.fromList [(CoreNodeId 2, NodeRekey)])]
            , setupSlotLength = defaultSlotLength
            , setupVersion = (minBound, ByronNodeToNodeVersion1)
            }
                            ...
                            =
                              NodeTopology
                                ( Map.fromList
                                    [ (CoreNodeId 0, Set.fromList [])
                                    , (CoreNodeId 1, Set.fromList [CoreNodeId 0])
                                    , (CoreNodeId 2, Set.fromList [CoreNodeId 0, CoreNodeId 1])
                                    , (CoreNodeId 3, Set.fromList [CoreNodeId 0, CoreNodeId 1, CoreNodeId 2])
                                    , (CoreNodeId 4, Set.fromList [CoreNodeId 0, CoreNodeId 1, CoreNodeId 2])
                                    ]
                                )
                          , initSeed = Seed 0
                          }
                    , setupNodeJoinPlan =
                        NodeJoinPlan
                          ( Map.fromList
                              ...
                              , (CoreNodeId 1, SlotNo{unSlotNo = 0})
                              , (CoreNodeId 2, SlotNo{unSlotNo = 0})
                              , (CoreNodeId 3, SlotNo{unSlotNo = 37})
                              , (CoreNodeId 4, SlotNo{unSlotNo = 37})
                              ]
                          )
                    , setupNodeRestarts =
                        NodeRestarts (Map.fromList [(SlotNo{unSlotNo = 37}, Map.fromList [(CoreNodeId 4, NodeRekey)])])
                    , setupSlotLength = defaultSlotLength
                    , setupVersion = (minBound, ByronNodeToNodeVersion1)
                    }
          ...
          let ncn = NumCoreNodes 3
           in prop_simple_real_pbft_convergence
                TestSetup
                  ...
                  , setupNodeJoinPlan = trivialNodeJoinPlan ncn
                  , setupNodeRestarts =
                      NodeRestarts $ Map.singleton (SlotNo 30) $ Map.singleton (CoreNodeId 2) NodeRekey
                  , setupSlotLength = defaultSlotLength
                  , setupVersion = (minBound, ByronNodeToNodeVersion1)
                  }
          ...
          prop_simple_real_pbft_convergence
            TestSetup
              ...
              , setupNodeJoinPlan =
                  NodeJoinPlan
                    ( Map.fromList
                        [ (CoreNodeId 0, SlotNo{unSlotNo = 2})
                        , (CoreNodeId 1, SlotNo{unSlotNo = 6})
                        , (CoreNodeId 2, SlotNo{unSlotNo = 9})
                        ]
                    )
              , setupNodeRestarts = noRestarts
              , setupSlotLength = slotLengthFromSec 20
        ...
        once $
          prop_simple_real_pbft_convergence
            TestSetup
              ...
              , setupNodeJoinPlan =
                  NodeJoinPlan
                    (Map.fromList [(CoreNodeId 0, SlotNo{unSlotNo = 0}), (CoreNodeId 1, SlotNo{unSlotNo = 33})])
              , setupNodeRestarts = noRestarts
              , setupSlotLength = slotLengthFromSec 20
              , setupVersion = (minBound, ByronNodeToNodeVersion1)
              }
                  ...
                  Rekeying
                    ...
                    , rekeyFreshSKs =
                        let prj = Crypto.hashVerKey . Crypto.deriveVerKeyDSIGN
                            acc0 =
                              -- the VKs of the operational keys at genesis
                              Set.fromList $
        ...
        conjoin
          [ counterexample (show (cid, pvuLabel)) $
              let ProtocolVersionUpdateLabel
               ...
               in property $ case pvuRequired of
        ...
        conjoin
          [ counterexample (show (cid, svuLabel)) $
              let SoftwareVersionUpdateLabel
               ...
               in property $ case svuRequired of
