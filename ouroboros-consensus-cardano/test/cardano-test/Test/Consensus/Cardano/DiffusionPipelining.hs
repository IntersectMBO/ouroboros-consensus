...
tests :: TestTree
tests = testGroup "Cardano diffusion pipelining"
...
instance All GenTentativeHeaderViews xs => GenTentativeHeaderViews (HardForkBlock xs) where
  genTentativeHeaderViews _ =
       ...
       .  foldMap hsequence'
       .  hcollapse . hap injections
      <$> gen
    where
      gen :: Gen (NP ([] :.: WrapTentativeHeaderView) xs)
      gen = hctraverse' (Proxy @GenTentativeHeaderViews)
  ...
  genTentativeHeaderViews _
    ...
    | otherwise = do
        ...
        hotIdentities <- concat <$> for issuerHashes \issuerHash -> do
          ...
          -- enforce these exact constraints here across different block numbers
