...
-- TODO remove once we update fs-sim
(</>) :: FsPath -> Text -> FsPath
a </> b = mkFsPath (Prelude.map unpack $ fsPathToList a ++ [b])
...
checkSnapshot m p (SomeHasFS fs) = case m of
  ...
  where
    ...
    hasTables tb = (doesFileExist,      ["tables", tb], "DOES NOT contain a \"tables/" <> Text.unpack tb <> "\" file")

    newFormatCheck tb =
        mapM_
            (\(doCheck, extra, err) -> want (doCheck fs) (foldl' (</>) p extra) err)
            [ isDir
            , hasTablesDir
            , hasState
            , hasTables tb
...
load Config{from = Mem, inpath} fs@(SomeHasFS hasFS) ccfg = do
    ...
    eExtLedgerSt <- runExceptT $ readExtLedgerState fs (decodeDiskExtLedgerState ccfg) decode (inpath </> "state")
    case eExtLedgerSt of
      Left err -> throwIO $ SnapshotError err
      Right extLedgerSt -> do
        values <- withFile hasFS (inpath </> "tables" </> "tvar") ReadMode $ \h -> do
            bs <- hGetAll hasFS h
...
load Config{from = LMDB, inpath} fs ccfg = do
    ...
    case eExtLedgerSt of
      ...
      Right extLedgerSt -> do
        values <- do
            dbEnv <- LMDB.openEnvironment (fsToFilePath (MountPoint ".") (inpath </> "tables")) defaultLMDBLimits
...
store Config{to = Mem, outpath} fs@(SomeHasFS hasFS) ccfg state = do
    ...
    writeExtLedgerState fs (encodeDiskExtLedgerState ccfg) (outpath </> "state") (forgetLedgerTables state)
    -- write tables
    createDirectoryIfMissing hasFS True $ outpath </> "tables"
    withFile hasFS (outpath </> "tables" </> "tvar") (WriteMode MustBeNew) $ \hf ->
        void $
            hPutAll hasFS hf $
...
store Config{to = LMDB, outpath} fs@(SomeHasFS hasFS) ccfg state = do
    ...
    -- write tables
    createDirectoryIfMissing hasFS True $ outpath </> "tables"
    dbEnv <- LMDB.openEnvironment (fsToFilePath (MountPoint ".") $ outpath </> "tables") defaultLMDBLimits
