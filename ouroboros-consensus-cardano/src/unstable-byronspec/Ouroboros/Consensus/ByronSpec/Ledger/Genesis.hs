...
instance SerialiseNodeToClient blk ByronSpecGenesis where
  encodeNodeToClient _ _ (ByronSpecGenesis delegators utxo pparams k slotLength) =
    mconcat
      [ encodeListLen 5
      , toCBOR delegators
      , encode utxo
...
modPParamsPBftThreshold ::
  ...
  Spec.PParams ->
  Spec.PParams
modPParamsPBftThreshold f pparams =
  pparams
