...
import Cardano.Protocol.TPraos.Rules.Overlay qualified as SL
...
import Ouroboros.Consensus.HardFork.History.Util qualified as Util
...
import Test.ThreadNet.Util.NodeTopology qualified as Topo
...
genPartition (NumCoreNodes n) (NumSlots t) (SecurityParam k') = do
    ...
    frequency $
      map (\(wgt, mbDur) -> (wgt, pure mbDur)) $
        filter
          ...
          ,
            ( 20
            , assert (numFirstEraEpochs == (1 :: Int)) $
                Just $
  ...
  let
    -- Because of FACT (A), we require there to be at least one slot after
...
mkMessageDelay part = CalcMessageDelay $
  \(CoreNodeId i, CoreNodeId j) curSlot _hdr ->
    NumSlots $
      if
