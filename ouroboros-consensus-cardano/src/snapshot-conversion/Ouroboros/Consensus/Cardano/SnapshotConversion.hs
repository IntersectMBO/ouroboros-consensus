...
data InEnv backend = InEnv
  ...
  -- ^ A progress message, just for displaying if we want the @interactive@
  -- version of the process. This is the default in @snapshot-converter@ and is
...
data OutEnv backend = OutEnv
  ...
  -- ^ In case some other directory needs to be wiped out, in particular the LSM
  -- directory which will be outside of the LedgerDB directory.
...
data SomeBackend args where
  SomeBackend ::
    StreamingBackend IO backend (LedgerState (CardanoBlock StandardCrypto)) =>
    args IO backend (LedgerState (CardanoBlock StandardCrypto)) -> SomeBackend args
