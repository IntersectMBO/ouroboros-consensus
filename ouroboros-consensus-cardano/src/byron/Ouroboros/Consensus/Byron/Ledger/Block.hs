...
import Ouroboros.Consensus.Byron.EBBs qualified as EBBs
...
instance ShowProxy (Header ByronBlock)
...
knownEBBs = Map.fromList $ map aux EBBs.knownEBBs
 where
  aux ::
    (CC.HeaderHash, Maybe CC.HeaderHash) ->
...
mkBoundaryUnsizedHeader ::
  SlotNo ->
  CC.ABoundaryHeader ByteString ->
  UnsizedHeader
mkBoundaryUnsizedHeader slotNo hdr =
  UnsizedHeader
    { unsizedHeaderRaw = hdr'
    , unsizedHeaderSlotNo = slotNo
    , unsizedHeaderHash = mkByronHash hdr'
...
joinSizeHint UnsizedHeader{..} size =
  ByronHeader
