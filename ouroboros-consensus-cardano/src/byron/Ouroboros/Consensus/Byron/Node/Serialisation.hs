...
instance EncodeDisk ByronBlock (PureLedgerState ByronBlock) where
  encodeDisk _ = encodeByronLedgerState
instance DecodeDisk ByronBlock (PureLedgerState ByronBlock) where
...
instance EncodeDiskDepIx (NestedCtxt Header) ByronBlock where
  encodeDiskDepIx _ccfg (SomeSecond (NestedCtxt ctxt)) =
    mconcat
      [ CBOR.encodeListLen 2
      , case ctxt of
          CtxtByronBoundary size ->
            mconcat
              [ CBOR.encodeWord8 0
              , CBOR.encodeWord32 (getSizeInBytes size)
              ]
          CtxtByronRegular size ->
            mconcat
              [ CBOR.encodeWord8 1
              , CBOR.encodeWord32 (getSizeInBytes size)
              ]
...
instance DecodeDiskDep (NestedCtxt Header) ByronBlock where
  decodeDiskDep ByronCodecConfig{..} (NestedCtxt ctxt) =
   ...
   where
    auxBoundary ::
      (Lazy.ByteString -> RawBoundaryHeader) ->
