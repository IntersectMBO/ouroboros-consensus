...
import Cardano.Chain.Byron.API qualified as ByronAPI
import Cardano.Chain.Genesis qualified as Genesis
import Cardano.Chain.MempoolPayload qualified as MempoolPayload
...
import Cardano.Chain.Update.Proposal qualified as Proposal
import Cardano.Chain.Update.Validation.Interface qualified as Update
import Cardano.Chain.Update.Validation.Registration qualified as Registration
import Cardano.Chain.Update.Vote (AVote)
import Cardano.Chain.Update.Vote qualified as Vote
  ...
  ldgr =
   ...
   where
    ...
    quorum =
      (\x -> assert (x > 0) x) $
        fromIntegral $
          Update.upAdptThd (fromIntegral n) pp0
     where
              ...
              if s < finalVoteSlot + twoK
                then st -- ignore endorsements until final vote is stable
                  ...
                  let ends' = Set.insert (Ref.mkLeaderOf params s) ends
                   in if fromIntegral (Set.size ends) < quorum
