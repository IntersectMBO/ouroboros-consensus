...
import Cardano.Api.KeysShelley (PaymentKey, SigningKey (PaymentSigningKey))
...
import Lens.Micro ((&), (.~), (^.))
import Ouroboros.Consensus.Block (Proxy (..), SlotNo)
...
import Test.ThreadNet.Util.NodeTopology (NodeTopology (..))
import Test.ThreadNet.Util.Seed (Seed (..))
...
getUTxOs =
  Map.mapKeys SL.getOriginalTxIn
    . getValuesMK
    . getLedgerTables
    . SL.shelleyLedgerTables
...
buildTx ::
  ShelleyBasedEra era => (SL.Tx Core.TopTx era -> SL.Tx Core.TopTx era) -> SL.Tx Core.TopTx era
buildTx mkTx =
  let
