...
-- forging credentials named on the command line.
--
-- The reading is done elsewhere: @cardano-config@ resolves the configuration and
-- loads every era's genesis, and "Cardano.Tools.Credentials" decodes the
-- credentials. What is left here is the mapping onto the consensus types.
--
-- A problem with either is thrown as a 'Cardano.Tools.Config.ConfigError', as in
-- the other tools, so the user gets a message rather than a backtrace.
--
-- The Shelley genesis comes back too because 'synthesize' needs its epoch
-- length.
initialize ::
