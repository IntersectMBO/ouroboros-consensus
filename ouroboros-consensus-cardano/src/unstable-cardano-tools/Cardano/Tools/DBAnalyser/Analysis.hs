...
storeLedgerStateAt slotNo ledgerAppMode env = do
 ...
 where
  ...
  process _ blk = do
    ...
    frk <-
      LedgerDB.getForkerAtTarget initLedgerDB "storeLedgerStateAt" registry VolatileTip >>= \case
  ...
  (AnalysisEnv{db, registry, startFrom, cfg, limit}) = do
   ...
   where
    ...
    process ledgerDB _ blk = do
      frk <-
        LedgerDB.getForkerAtTarget ledgerDB "traceLedgerProcessing" registry VolatileTip >>= \case
          Left{} -> error "Unreachable, volatile tip MUST be in the LedgerDB"
          Right f -> pure f
...
getBlockApplicationMetrics (NumberOfBlocks nrBlocks) mOutFile env = do
 ...
 where
  ...
  process ledgerDB intLedgerDB outFileHandle _ blk = do
    frk <-
      LedgerDB.getForkerAtTarget ledgerDB "getBlockApplicationMetrics" registry VolatileTip >>= \case
        Left{} -> error "Unreachable, volatile tip MUST be in the LedgerDB"
        Right f -> pure f
    oldLedgerSt <- IOLike.atomically $ LedgerDB.forkerGetLedgerState frk
    oldLedgerTbs <- LedgerDB.forkerReadTables frk (getBlockKeySets blk)
              ...
              pure $
                MempoolLedgerDBView
                  ...
                  ( fmap (LedgerDB.ledgerStateReadOnlyForker . LedgerDB.readOnlyForker)
                      <$> LedgerDB.getForkerAtTarget
                        ledgerDB
                        "reproMempoolForge:Mempool"
                        reg'
  ...
  process howManyBlocks mempool mbBlk blk' =
    (\() -> Just blk') <$> do
      ...
      case scrutinee of
        Nothing -> pure ()
        Just blk -> do
          LedgerDB.withPrivateTipForker ledgerDB "reproMempoolForge:forger" $ \forker -> do
