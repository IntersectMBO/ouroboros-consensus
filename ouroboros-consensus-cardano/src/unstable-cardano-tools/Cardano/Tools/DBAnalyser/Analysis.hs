        ...
        Mempool.LedgerInterface
          { Mempool.getCurrentLedgerState = do
              ...
              pure $
                MempoolLedgerDBView
                  (ledgerState st)
                  ( fmap (second (fmap LedgerDB.ledgerStateReadOnlyForker)) $
                      allocate
                        reg
                        (\_ -> LedgerDB.unsafeGetReadOnlyForker ledgerDB (SpecificPoint (castPoint $ getTip st)))
