...
dispatch ::
  LedgerState (CardanoBlock StandardCrypto) Values ->
  (LedgerState ByronBlock Values -> IO TextBuilder) ->
  (forall proto era. LedgerState (ShelleyBlock proto era) Values -> IO TextBuilder) ->
  IO TextBuilder
dispatch cardanoSt fByron fShelley =
 ...
 where
  ...
  k_fShelley ::
    forall proto era.
    Flip LedgerState Values (ShelleyBlock proto era) ->
