...
inMemoryBackendArgs = LedgerDbBackendArgs $ \tr shfs ->
  let
    mkH :: MkHandle m
    mkH = mkInMemoryFactory tr shfs

    mkHs :: MkHandleFromSnapshot m
    mkHs = mkInMemoryFromSnapshot tr shfs
   in
    pure
      BackendResources
...
loadSnapshot expectedBackend mkFromSnapshot mkH ccfg fs@(SomeHasFS hfs) ds = do
  ...
  case pointToWithOriginRealPoint (getTip ls) of
    ...
    NotOrigin pt -> do
      ...
      let computedCRC = maybe checksumAsRead (crcOfConcat checksumAsRead) mbCrcTables
