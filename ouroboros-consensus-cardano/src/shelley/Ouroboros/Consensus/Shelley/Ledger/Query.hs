...
data instance BlockQuery (ShelleyBlock proto era) fp result where
  ...
  GetConstitutionHash :: BlockQuery (ShelleyBlock proto era) QFNoTables (Maybe (SafeHash (EraCrypto era) ByteString))
...
instance SameDepIndex2 (BlockQuery (ShelleyBlock proto era)) where
  ...
  sameDepIndex2 (GetStakeDelegDeposits _) _
    = Nothing
  sameDepIndex2 GetConstitutionHash GetConstitutionHash = Just Refl
  sameDepIndex2 GetConstitutionHash _ = Nothing
...
decodeShelleyQuery = do
    ...
    case (len, tag) of
      ...
      (1, 23) -> return $ SomeBlockQuery GetConstitutionHash
      _       -> fail $
