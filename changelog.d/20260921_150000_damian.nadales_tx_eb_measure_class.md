...
- Added `TxEbMeasure`, `txEbMeasure`, `ebCapacityTxMeasure` and `mempoolEbReservation` to the `TxLimits` class.
  ...
  The references capacity is `maxEndorserBlockReferencesSize` minus `encodedMaxFramingSize`, the largest header that `encodeEndorserBlock` writes before the references.
