...
- This narrows `NodeToClientV_23` itself: a peer that sent CBOR tag 4, 5 or 21,
  ...
  `GetLedgerPeerSnapshot'` applied to `False`, whereas the `GetLedgerPeerSnapshot`
  pattern synonym that `cardano-api` and `dmq-node` use encodes the two-element
  form. A client too old to know that form offers at most `NodeToClientV_22` and
  now stops at the handshake instead.
